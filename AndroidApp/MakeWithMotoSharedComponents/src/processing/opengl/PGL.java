...
public class PGL {
  ...
  protected void updatePrimary() {    
    if (!initialized) {      
      ...
      if (ext.indexOf("texture_non_power_of_two") > -1) {
        fboWidth = pg.width;
        fboHeight = pg.height;
      } else {
  ...
  public void drawElements(int mode, int count, int type, int offset) {
      // GLES20 Fix!
	  fix.android.opengl.GLES20.glDrawElements(mode, count, type, offset);
  }
  ...
  public void vertexAttribPointer(int loc, int size, int type, boolean normalized, int stride, int offset) {
      // GLES20 Fix!
	  fix.android.opengl.GLES20.glVertexAttribPointer(loc, size, type, normalized, stride, offset);    
  }
