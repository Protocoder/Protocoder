...
/**
 * <code>OSCPacket</code> is the superclass of OSC messages and bundles. It provides methods that apply to both of these
 * OSC packet types, like calculating the packet's size encoding the packet from a given byte buffer or decoding a
 * received message from a given buffer.
 * 
...
public abstract class OSCPacket {
    ...
    /**
     * Constructs a new <code>OSCPacket</code>. Refer to the subclasses for the particular way they instantiate the
     * class
     */
     ...
     * position, using the default codec. To write the encoded packet, you will typically call <code>flip()</code> on
    ...
    /**
     * Encodes the contents of this packet into the provided <code>ByteBuffer</code>, beginning at the buffer's current
     * position, using a given codec. To write the encoded packet, you will typically call <code>flip()</code> on the
     * buffer, then <code>write()</code> on the channel.
     * 
     * @param b
     *            <code>ByteBuffer</code> pointing right at the beginning of the osc packet. buffer position will be
     *            right after the end of the packet when the method returns.
    ...
    /**
     * Creates a new packet decoded from the ByteBuffer, using the default codec. This method simply calls the
     * equivalent method in <code>OSCPacketCodec</code>.
     * 
     * @param b
    ...
    /**
     * Prints a text version of a packet to a given stream. The format is similar to scsynth using dump mode 1. Bundles
     * will be printed with each message on a separate line and increasing indent.
     * 
     * @param stream
    ...
    /**
     * Prints a hexdump version of a packet to a given stream. The format is similar to scsynth using dump mode 2.
     * Unlike <code>printTextOn</code> this takes a raw received or encoded byte buffer and not a decoded instance of
     * <code>OSCPacket</code>.
     * 
     * @param stream
     *            the print stream to use, for example <code>System.out</code>
     * @param b
     *            the byte buffer containing the packet. read position should be at the very beginning of the packet,
     *            limit should be at the end of the packet. this method alters the buffer position in a manner that a
     *            successive <code>flip()</code> will restore the original position and limit.
     * 
