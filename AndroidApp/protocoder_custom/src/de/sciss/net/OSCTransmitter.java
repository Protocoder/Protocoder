...
public abstract class OSCTransmitter implements OSCChannel {
    ...
    /**
     * Creates a new instance of an <code>OSCTransmitter</code>, using default codec and a specific transport protocol.
     * It picks an arbitrary free port and uses the local machine's IP. To determine the resulting port, you can use
    ...
    /**
     * Creates a new instance of an <code>OSCTransmitter</code>, using a specific codec and transport protocol. It picks
     * an arbitrary free port and uses the local machine's IP. To determine the resulting port, you can use
     * <code>getLocalAddress</code> afterwards.
     * 
     * @param c
     *            the codec to use
    ...
    /**
     * Creates a new instance of an <code>OSCTransmitter</code>, using default codec and a specific transport protocol
     * and port. It uses the local machine's IP. Note that the <code>port</code> specifies the local socket, not the
     * remote (or target) port. This can be set using the <code>setTarget</code> method!
     * 
     * @param protocol
     *            the protocol to use, currently either <code>UDP</code> or <code>TCP</code>
     * @param port
    ...
    /**
     * Creates a new instance of an <code>OSCTransmitter</code>, using a specific codec and transport protocol and port.
     * It uses the local machine's IP. Note that the <code>port</code> specifies the local socket, not the remote (or
     * target) port. This can be set using the <code>setTarget</code> method!
     * 
     * @param c
     *            the codec to use
     * @param protocol
    ...
    /**
     * Creates a new instance of an <code>OSCTransmitter</code>, using default codec and a specific transport protocol
     * and port. It uses the local machine's IP or the &quot;loopback&quot; address. Note that the <code>port</code>
     * specifies the local socket, not the remote (or target) port. This can be set using the <code>setTarget</code>
     * method!
     * 
     * @param protocol
     *            the protocol to use, currently either <code>UDP</code> or <code>TCP</code>
    ...
    /**
     * Creates a new instance of an <code>OSCTransmitter</code>, using a specific codec and transport protocol and port.
     * It uses the local machine's IP or the &quot;loopback&quot; address. Note that the <code>port</code> specifies the
     * local socket, not the remote (or target) port. This can be set using the <code>setTarget</code> method!
     * 
     * @param c
     *            the codec to use
     * @param protocol
    ...
    /**
     * Creates a new instance of an <code>OSCTransmitter</code>, using default codec and a specific transport protocol
     * and local socket address. Note that <code>localAddress</code> specifies the local socket, not the remote (or
     * target) socket. This can be set using the <code>setTarget</code> method!
     * 
     * @param protocol
     *            the protocol to use, currently either <code>UDP</code> or <code>TCP</code>
     * @param localAddress
     *            a valid address to use for the OSC socket. If the port is <code>0</code>, an arbitrary free port is
     *            picked when the transmitter is connected. (you can find out the actual port in this case by calling
     *            <code>getLocalAddress()</code> after the transmitter was connected).
     * 
     * @return the newly created transmitter
     * 
     * @throws IOException
    ...
    /**
     * Creates a new instance of an <code>OSCTransmitter</code>, using a specific codec and transport protocol and local
     * socket address. Note that <code>localAddress</code> specifies the local socket, not the remote (or target)
     * socket. This can be set using the <code>setTarget</code> method!
     * 
     * @param c
     *            the codec to use
     * @param protocol
    ...
    /**
     * Creates a new instance of an <code>OSCTransmitter</code>, using default codec and UDP transport on a given
     * channel. The caller should ensure that the provided channel's socket was bound to a valid address (using
    ...
    /**
     * Creates a new instance of an <code>OSCTransmitter</code>, using a specific codec and UDP transport on a given
     * channel. The caller should ensure that the provided channel's socket was bound to a valid address (using
     * <code>dch.socket().bind( SocketAddress )</code>). Note that <code>dch</code> specifies the local socket, not the
     * remote (or target) socket. This can be set using the <code>setTarget</code> method!
     * 
    ...
    /**
     * Creates a new instance of an <code>OSCTransmitter</code>, using default codec and TCP transport on a given
     * channel. The caller should ensure that the provided channel's socket was bound to a valid address (using
    ...
    /**
     * Creates a new instance of an <code>OSCTransmitter</code>, using a specific codec and TCP transport on a given
     * channel. The caller should ensure that the provided channel's socket was bound to a valid address (using
     * <code>sch.socket().bind( SocketAddress )</code>). Furthermore, the channel must be connected (using
     * <code>connect()</code>) before being able to transmit messages. Note that <code>sch</code> specifies the local
     * socket, not the remote (or target) socket. This can be set using the <code>setTarget</code> method!
    ...
    /**
     * Queries the transmitter's local socket address. You can determine the host and port from the returned address by
     * calling <code>getHostName()</code> (or for the IP <code>getAddress().getHostAddress()</code>) and
     * <code>getPort()</code>. This port number may be <code>0</code> if the transmitter was called with an unspecified
     * port and has not yet been connected. In this case, to determine the port actually used, call this method after
     * the transmitter has been connected.
     * 
     ...
     * 
     * @param target
     *            the address of the remote target socket. Usually you construct an appropriate
     ...
     * <code>OSCTransmitter.newUsing( &quot;udp&quot; )</code>, you are required to call <code>connect()</code> so that
     * an actual <code>DatagramChannel</code> is created and bound. For a <B>UDP</B> transmitter which was created with
     * an explicit <code>DatagramChannel</code>, this method does noting, so it is always safe to call
     * <code>connect()</code>. However, for <B>TCP</B> transmitters, this may throw an <code>IOException</code> if the
     * transmitter was already connected, therefore be sure to check <code>isConnected()</code> before.
     * 
     * @throws IOException
     *             if a networking error occurs. Possible reasons: - the underlying network channel had been closed by
     *             the server. - the transport is TCP and the server is not available. - the transport is TCP and an
     *             <code>OSCReceiver</code> sharing the same socket was stopped before (unable to revive).
     * 
     * @see #isConnected()
     */
     ...
     * 
     * @return <code>true</code> if the transmitter is connected, <code>false</code> otherwise. For transports that do
     ...
     * 
     * @throws IOException
     *             if a write error, OSC encoding error, buffer overflow error or network error occurs
     */
     ...
     *             if no default address was specified
     * 
    ...
    private static class UDPOSCTransmitter extends OSCTransmitter {
	...
	public void send(OSCPacketCodec c, OSCPacket p, SocketAddress target) throws IOException {
		...
		throw new OSCException(OSCException.BUFFER, p instanceof OSCMessage ? ((OSCMessage) p).getName() : p
			.getClass().getName());
	    }
	}
    }

    private static class TCPOSCTransmitter extends OSCTransmitter {
	...
	protected TCPOSCTransmitter(OSCPacketCodec c, SocketChannel sch) {
	    ...
	    if (sch.isConnected())
		setTarget(sch.socket().getRemoteSocketAddress());
	}
	...
	public void dispose() {
		...
		try {
		    ...
		    // new IOException( "TCPOSCTransmitter.closeChannel()"
		    // ).printStackTrace();
		    sch.close();
