...
public class OSCClient implements OSCBidi {
    ...
    /**
     * Creates a new instance of an <code>OSCClient</code>, using default codec and a specific transport protocol. It
    ...
    /**
     * Creates a new instance of an <code>OSCClient</code>, using a specific codec and transport protocol. It picks an
    ...
    /**
     * Creates a new instance of an <code>OSCClient</code>, using default codec and a specific transport protocol and
     * port. It uses the local machine's IP.
     * <p>
     * Note that the <code>port</code> specifies the local socket (at which the client listens and from which it sends),
     * it does not determine the remote sockets from which messages can be received and to which messages are sent. The
     * target socket can be set using the <code>setTarget</code> method!
     * 
     * @param protocol
     *            the protocol to use, currently either <code>UDP</code> or <code>TCP</code>
     * @param port
    ...
    /**
     * Creates a new instance of an <code>OSCClient</code>, using a specific codec and transport protocol and port. It
    ...
    /**
     * Creates a new instance of an <code>OSCClient</code>, using default codec and a specific transport protocol and
     * port. It uses the local machine's IP or the &quot;loopback&quot; address.
     * <p>
     * Note that the <code>port</code> specifies the local socket (at which the client listens and from which it sends),
     * it does not determine the remote sockets from which messages can be received and to which messages are sent. The
     * target socket can be set using the <code>setTarget</code> method!
    ...
    /**
     * Creates a new instance of an <code>OSCClient</code>, using default codec and a specific transport protocol and
     * local socket address. Note that <code>localAdress</code> specifies the local socket (at which the receiver
     * listens and from which the transmitter sends), it does not determine the remote sockets to which the client
     * connects. To specify the remote socket, use the <code>setTarget</code> method!
     * <P>
     * 
     * @param protocol
     *            the protocol to use, currently either <code>UDP</code> or <code>TCP</code>
    ...
    /**
     * Creates a new instance of an <code>OSCClient</code>, using a given codec, a specific transport protocol and local
     * socket address. Note that <code>localAdress</code> specifies the local socket (at which the receiver listens and
     * from which the transmitter sends), it does not determine the remote sockets to which the client connects. To
     * specify the remote socket, use the <code>setTarget</code> method!
     * <P>
     * 
     * @param c
     *            the codec to use
    ...
    /**
     * Queries the client side socket address. This is the address from which the client sends and at which it listens
     * for replies. You can determine the host and port from the returned address by calling <code>getHostName()</code>
     * (or for the IP <code>getAddress().getHostAddress()</code>) and <code>getPort()</code>.
     * <p>
     * Note that if the client is bound to the accept-any IP <code>"0.0.0.0"</code>, which happens for example when
     ...
     * 
     * @param target
     *            the address of the server. Usually you construct an appropriate <code>InetSocketAddress</code>
     * 
     ...
     *             the server. - the transport is TCP and the server is not available. - the transport is TCP and the
     *             client was stopped before (unable to revive).
     * 
     * @see #isConnected()
     * @see #start()
     */
     ...
     * 
     * @throws IOException
     *             if a write error, OSC encoding error, buffer overflow error or network error occurs, for example if a
     *             TCP client was not connected before.
     ...
     *             if trying to change the buffer size while the client is active (listening).
     * 
     * @see #isActive()
     * @see #getBufferSize()
     */
    ...
    /**
     * Queries the buffer size used for sending and receiving OSC messages. This is the maximum size an OSC packet
     * (bundle or message) can grow to. The initial buffer size is <code>DEFAULTBUFSIZE</code>.
     * 
     ...
     * 
     * @param mode
     *            one of <code>kDumpOff</code> (don't dump, default), <code>kDumpText</code> (dump human readable
     *            string), <code>kDumpHex</code> (hexdump), or <code>kDumpBoth</code> (both text and hex)
     * @param stream
     *            the stream to print on, or <code>null</code> which is shorthand for <code>System.err</code>
     * 
