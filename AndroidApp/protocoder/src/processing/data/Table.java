...
public class Table implements Iterable<TableRow> {
	...
	public void writeHTML(PrintWriter writer) {
		...
		for (int row = 0; row < getRowCount(); row++) {
			...
			for (int col = 0; col < getColumnCount(); col++) {
				...
				// String clean = (entry == null) ? "" :
				// HTMLFairy.encodeEntities(entry);
	...
	/**
	 * Write this table as a TSV file. Exceptions will be printed, but not
	 * thrown.
	 * 
	 * @param file
	 ...
	 *            Name of the column.
	 * @param report
	 *            Whether to print to System.err if the column wasn't found.
	 ...
	 * the rows of this table. This is very efficient, but not very thread-safe.
	 * If you want to iterate in a multi-threaded manner, use createIterator().
	 */
	...
	/**
	 * Set all 'null' entries to "" (zero length String objects). If columns are
	 * typed, then this will only apply to String columns.
	 */
	...
	/**
	 * Set all "" entries (zero length String objects) to null values. If
	 * columns are typed, then this will only apply to String columns.
	 */
	...
	/**
	 * Searches the entire table for float values. Returns missing float
	 * (Float.NaN by default) if no valid numbers found.
	 */
	...
	/**
	 * Removed any of the specified characters from a column. For instance, the
	 * following code removes dollar signs and commas from column 2:
	 * 
	 * <pre>
	...
	/**
	 * Run String.replaceAll() on all entries in a column. Only works with
	 * columns that are already String values.
	 * 
	 * @param what
	 *            the String to match
	 * @param columnName
