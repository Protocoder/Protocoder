 ...
 * vector format so it allows for infinite resolution and relatively small file
 * sizes. Most modern media software can view SVG files, including Adobe
 * products, Firefox, etc. Illustrator and Inkscape can edit SVG files.
 * <p>
 * We have no intention of turning this into a full-featured SVG library. The
 * goal of this project is a basic shape importer that is small enough to be
 * included with applets, meaning that its download size should be in the
 * neighborhood of 25-30k. Starting with release 0149, this library has been
 * incorporated into the core via the loadShape() command, because vector shape
 * data is just as important as the image data from loadImage().
 * <p>
 * For more sophisticated import/export, consider the <A
 * HREF="http://xmlgraphics.apache.org/batik/">Batik</A> library from the Apache
 * Software Foundation. Future improvements to this library may focus on this
 * properly supporting a specific subset of SVG, for instance the simpler SVG
 * profiles known as <A HREF="http://www.w3.org/TR/SVGMobile/">SVG Tiny or
 * Basic</A>, although we still would not support the interactivity options.
 * 
 * <p>
 * <hr noshade>
 * <p>
 ...
 * </UL>
 * 
 * Some SVG objects and features may not yet be supported. Here is a partial
 * list of non-included features
 * <UL>
 ...
 * </UL>
 * 
 * For those interested, the SVG specification can be found <A
 * HREF="http://www.w3.org/TR/SVG">here</A>.
 */
public class PShapeSVG extends PShape {
	...
	protected PShape parseChild(XML elem) {
		...
		if (name == null) {
			...
			PGraphics.showWarning("Text and fonts in SVG files " + "are not currently supported, "
	...
	protected void parseLine() {
		kind = LINE;
		family = PRIMITIVE;
		params = new float[] { getFloatWithUnit(element, "x1"), getFloatWithUnit(element, "y1"),
	...
	protected void parseRect() {
		kind = RECT;
		family = PRIMITIVE;
		params = new float[] { getFloatWithUnit(element, "x"), getFloatWithUnit(element, "y"),
	...
	protected void parsePath() {
		...
		for (int i = 0; i < pathDataChars.length; i++) {
			...
			if (c == 'M' || c == 'm' || c == 'L' || c == 'l' || c == 'H' || c == 'h' || c == 'V' || c == 'v'
					|| c == 'C' || c == 'c' || // beziers
	...
	class LinearGradient extends Gradient {
		...
		public LinearGradient(PShapeSVG parent, XML properties) {
			...
			if (transformStr != null) {
				...
				transform.setValues(new float[] { // TODO don't create temp
													// floats
						t[0], t[1], t[2], t[3], t[4], t[5], 0, 0, 1 });

				// Point2D t1 = transform.transform(new Point2D.Float(x1, y1),
				// null);
				// Point2D t2 = transform.transform(new Point2D.Float(x2, y2),
				// null);
	...
	class RadialGradient extends Gradient {
		...
		public RadialGradient(PShapeSVG parent, XML properties) {
			...
			if (transformStr != null) {
				...
				// null);
				// Point2D t2 = transform.transform(new Point2D.Float(cx + r,
				// cy), null);
	...
	/*
	 * class LinearGradientPaint implements Paint { float x1, y1, x2, y2;
	 * float[] offset; int[] color; int count; float opacity;
	 * 
	 * public LinearGradientPaint(float x1, float y1, float x2, float y2,
	 * float[] offset, int[] color, int count, float opacity) { this.x1 = x1;
	 * this.y1 = y1; this.x2 = x2; this.y2 = y2; this.offset = offset;
	 * this.color = color; this.count = count; this.opacity = opacity; }
	 * 
	 * public PaintContext createContext(ColorModel cm, Rectangle deviceBounds,
	 * Rectangle2D userBounds, AffineTransform xform, RenderingHints hints) {
	 * Point2D t1 = xform.transform(new Point2D.Float(x1, y1), null); Point2D t2
	 * = xform.transform(new Point2D.Float(x2, y2), null); return new
	 * LinearGradientContext((float) t1.getX(), (float) t1.getY(), (float)
	 * t2.getX(), (float) t2.getY()); }
	 * 
	 * public int getTransparency() { return TRANSLUCENT; // why not.. rather
	 * than checking each color }
	 * 
	 * public class LinearGradientContext implements PaintContext { int ACCURACY
	 * = 2; float tx1, ty1, tx2, ty2;
	 * 
	 ...
	 * 
	 * public Raster getRaster(int x, int y, int w, int h) { WritableRaster
	 * raster = getColorModel().createCompatibleWritableRaster(w, h);
	 * 
	 * int[] data = new int[w * h * 4];
	 * 
	 * // make normalized version of base vector float nx = tx2 - tx1; float ny
	 * = ty2 - ty1; float len = (float) Math.sqrt(nx*nx + ny*ny); if (len != 0)
	 * { nx /= len; ny /= len; }
	 * 
	 * int span = (int) PApplet.dist(tx1, ty1, tx2, ty2) * ACCURACY; if (span <=
	 * 0) { //System.err.println("span is too small"); // annoying edge case
	 * where the gradient isn't legit int index = 0; for (int j = 0; j < h; j++)
	 * { for (int i = 0; i < w; i++) { data[index++] = 0; data[index++] = 0;
	 * data[index++] = 0; data[index++] = 255; } }
	 * 
	 * } else { int[][] interp = new int[span][4]; int prev = 0; for (int i = 1;
	 * i < count; i++) { int c0 = color[i-1]; int c1 = color[i]; int last =
	 * (int) (offset[i] * (span-1)); //System.out.println("last is " + last);
	 * for (int j = prev; j <= last; j++) { float btwn = PApplet.norm(j, prev,
	 * last); interp[j][0] = (int) PApplet.lerp((c0 >> 16) & 0xff, (c1 >> 16) &
	 * 0xff, btwn); interp[j][1] = (int) PApplet.lerp((c0 >> 8) & 0xff, (c1 >>
	 * 8) & 0xff, btwn); interp[j][2] = (int) PApplet.lerp(c0 & 0xff, c1 & 0xff,
	 * btwn); interp[j][3] = (int) (PApplet.lerp((c0 >> 24) & 0xff, (c1 >> 24) &
	 * 0xff, btwn) * opacity); //System.out.println(j + " " + interp[j][0] + " "
	 * + interp[j][1] + " " + interp[j][2]); } prev = last; }
	 * 
	 * int index = 0; for (int j = 0; j < h; j++) { for (int i = 0; i < w; i++)
	 * { //float distance = 0; //PApplet.dist(cx, cy, x + i, y + j); //int which
	 * = PApplet.min((int) (distance * ACCURACY), interp.length-1); float px =
	 * (x + i) - tx1; float py = (y + j) - ty1; // distance up the line is the
	 * dot product of the normalized // vector of the gradient start/stop by the
	 * point being tested int which = (int) ((px*nx + py*ny) * ACCURACY); if
	 * (which < 0) which = 0; if (which > interp.length-1) which =
	 * interp.length-1; //if (which > 138) System.out.println("grabbing " +
	 * which);
	 * 
	 * data[index++] = interp[which][0]; data[index++] = interp[which][1];
	 * data[index++] = interp[which][2]; data[index++] = interp[which][3]; } } }
	...
	/*
	 * class RadialGradientPaint implements Paint { float cx, cy, radius;
	 * float[] offset; int[] color; int count; float opacity;
	 * 
	 * public RadialGradientPaint(float cx, float cy, float radius, float[]
	 * offset, int[] color, int count, float opacity) { this.cx = cx; this.cy =
	 * cy; this.radius = radius; this.offset = offset; this.color = color;
	 * this.count = count; this.opacity = opacity; }
	 * 
	 * public PaintContext createContext(ColorModel cm, Rectangle deviceBounds,
	 * Rectangle2D userBounds, AffineTransform xform, RenderingHints hints) {
	 ...
	 * 
	 * public class RadialGradientContext implements PaintContext { int ACCURACY
	 * = 5;
	 * 
	 * public void dispose() {}
	 * 
	 ...
	 * raster = getColorModel().createCompatibleWritableRaster(w, h);
	 * 
	 * int span = (int) radius * ACCURACY; int[][] interp = new int[span][4];
	 * int prev = 0; for (int i = 1; i < count; i++) { int c0 = color[i-1]; int
	 * c1 = color[i]; int last = (int) (offset[i] * (span - 1)); for (int j =
	 * prev; j <= last; j++) { float btwn = PApplet.norm(j, prev, last);
	 * interp[j][0] = (int) PApplet.lerp((c0 >> 16) & 0xff, (c1 >> 16) & 0xff,
	 * btwn); interp[j][1] = (int) PApplet.lerp((c0 >> 8) & 0xff, (c1 >> 8) &
	 * 0xff, btwn); interp[j][2] = (int) PApplet.lerp(c0 & 0xff, c1 & 0xff,
	 * btwn); interp[j][3] = (int) (PApplet.lerp((c0 >> 24) & 0xff, (c1 >> 24) &
	 * 0xff, btwn) * opacity); } prev = last; }
	 * 
	 * int[] data = new int[w * h * 4]; int index = 0; for (int j = 0; j < h;
	 * j++) { for (int i = 0; i < w; i++) { float distance = PApplet.dist(cx,
	 * cy, x + i, y + j); int which = PApplet.min((int) (distance * ACCURACY),
	 * interp.length-1);
	 * 
	 * data[index++] = interp[which][0]; data[index++] = interp[which][1];
	 * data[index++] = interp[which][2]; data[index++] = interp[which][3]; } }
	...
	protected Shader calcGradientPaint(Gradient gradient) {
		...
		if (gradient instanceof LinearGradient) {
			...
			// return new LinearGradientPaint(grad.x1, grad.y1, grad.x2,
			// grad.y2,
			// grad.offset, grad.color, grad.count,
			// opacity);
			return new android.graphics.LinearGradient(grad.x1, grad.y1, grad.x2, grad.y2, colors, grad.offset,
			...
			// grad.offset, grad.color, grad.count,
			// opacity);
			return new android.graphics.RadialGradient(grad.cx, grad.cy, grad.r, colors, grad.offset,
					Shader.TileMode.CLAMP);
		}
		return null;
	}
