...
public class GLUtessellatorImpl implements PGLUtessellator {
	...
	GLUmesh mesh; /*
				 * stores the input contours, and eventually the tessellation
				 * itself
				 */
	...
	boolean avoidDegenerateTris; /*
								 * JOGL-specific hint to try to improve
								 * triangulation by avoiding producing
								 * degenerate (zero-area) triangles; has not
								 * been tested exhaustively and is therefore an
								 * option
								 */
	...
	// private static final int GLU_TESS_MESH = 100112; /* void (*)(GLUmesh
	// *mesh) */
	...
	private void gotoState(int newState) {
		while (state != newState) {
			/*
			 * We change the current state one level at a time, to get to the
			 * desired state.
			 */
			if (state < newState) {
	...
	public void gluGetTessProperty(int which, double[] value, int value_offset) {
		...
		case PGLU.GLU_TESS_TOLERANCE:
			...
			break;
		case PGLU.GLU_TESS_WINDING_RULE:
			assert (windingRule == PGLU.GLU_TESS_WINDING_ODD || windingRule == PGLU.GLU_TESS_WINDING_NONZERO
					|| windingRule == PGLU.GLU_TESS_WINDING_POSITIVE || windingRule == PGLU.GLU_TESS_WINDING_NEGATIVE || windingRule == PGLU.GLU_TESS_WINDING_ABS_GEQ_TWO);
	...
	public void gluTessCallback(int which, PGLUtessellatorCallback aCallback) {
		...
		case PGLU.GLU_TESS_EDGE_FLAG:
			...
			/*
			 * If the client wants boundary edges to be flagged, we render
			 * everything as separate triangles (no strips or fans).
			 */
	...
	private boolean addVertex(double[] coords, Object vertexData) {
		...
		if (e == null) {
			...
			/*
			 * Create a new vertex and edge which immediately follow e in the
			 * ordering around the left face.
			 */
		 ...
		 * vertices in such an order that a CCW contour will add +1 to the
		 * winding number of the region inside the contour.
		 */
		e.winding = 1;
		e.Sym.winding = -1;
		...
		try {
			...
			if (this.mesh == null) {
				if (!flagBoundary /* && callMesh == NULL_CB */) {
					 ...
					 * contours, intersections, edge flags, and of course it
					 * does not generate an explicit mesh either.
					 */
			...
			/*
			 * __gl_computeInterior( tess ) computes the planar arrangement
			 * specified by the given contours, and further subdivides this
			 * arrangement into regions. Each region is marked "inside" if it
			 * belongs to the polygon, according to the rule given by
			 * windingRule. Each interior region is guaranteed be monotone.
			 */
			...
			if (!fatalError) {
				...
				/*
				 * If the user wants only the boundary contours, we throw away
				 * all edges except those which separate the interior from the
				 * exterior. Otherwise we tessellate all the regions marked
				 * "inside".
				 */
				if (boundaryOnly) {
				...
				if (callBegin != NULL_CB || callEnd != NULL_CB || callVertex != NULL_CB || callEdgeFlag != NULL_CB
						|| callBeginData != NULL_CB || callEndData != NULL_CB || callVertexData != NULL_CB
						|| callEdgeFlagData != NULL_CB) {
					if (boundaryOnly) {
				...
				// if (callMesh != NULL_CB) {
				//
				// /* Throw away the exterior faces, so that all faces are
				// interior.
				// * This way the user doesn't have to check the "inside" flag,
				// * and we don't need to even reveal its existence. It also
				// leaves
				// * the freedom for an implementation to not generate the
				// exterior
