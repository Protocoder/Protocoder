...
class PFontTexture implements PConstants {
	...
	public boolean addTexture() {
		...
		if (is3D) {
			...
			// ...however, the effect of bilinear sampling is to add some
			// blurriness to the text
			// in its original size. In 2D, we assume that text will be shown at
			// its original
			// size, so linear sampling is chosen instead (which only affects
			// minimized text).
	...
	protected void addToTexture(int idx, PFont.Glyph glyph) {
		...
		// fractional
		// screen positions. I.e.: the pixel on the screen only contains half of
		// the
		// font rectangle, so it would sample half of the color from the glyph
		// area in the texture, and the other half from the contiguous pixel. If
		// the
		// later contains a portion of the neighbor glyph and the former
		// doesn't, this
		// would result in a shaded pixel when the correct output is blank.
		// This is a consequence of putting all the glyphs in a common texture
		// with
		// bilinear sampling.
		int w = 1 + glyph.width + 1;
		int h = 1 + glyph.height + 1;

		// Converting the pixels array from the PImage into a valid RGBA array
		// for OpenGL.
		int[] rgba = new int[w * h];
		int t = 0;
		int p = 0;
		if (PGL.BIG_ENDIAN) {
			java.util.Arrays.fill(rgba, 0, w, 0xFFFFFF00); // Set the first row
															// to blank pixels.
			t = w;
			for (int y = 0; y < glyph.height; y++) {
				rgba[t++] = 0xFFFFFF00; // Set the leftmost pixel in this row as
										// blank
				for (int x = 0; x < glyph.width; x++) {
					rgba[t++] = 0xFFFFFF00 | glyph.image.pixels[p++];
				}
				rgba[t++] = 0xFFFFFF00; // Set the rightmost pixel in this row
										// as blank
			}
			java.util.Arrays.fill(rgba, (h - 1) * w, h * w, 0xFFFFFF00); // Set
																			// the
																			// last
																			// row
																			// to
			...
			java.util.Arrays.fill(rgba, 0, w, 0x00FFFFFF); // Set the first row
															// to blank pixels.
			t = w;
			for (int y = 0; y < glyph.height; y++) {
				rgba[t++] = 0x00FFFFFF; // Set the leftmost pixel in this row as
										// blank
				for (int x = 0; x < glyph.width; x++) {
					rgba[t++] = (glyph.image.pixels[p++] << 24) | 0x00FFFFFF;
				}
				rgba[t++] = 0x00FFFFFF; // Set the rightmost pixel in this row
										// as blank
			}
			java.util.Arrays.fill(rgba, (h - 1) * w, h * w, 0x00FFFFFF); // Set
																			// the
																			// last
																			// row
																			// to
		...
		if (offsetY + lineHeight > textures[currentTex].glHeight) {
			// We run out of space in the current texture, so we add a new
			// texture:
			resized = addTexture();
			if (resized) {
	...
	public class TextureInfo {
		...
		public TextureInfo(int tidx, int cropX, int cropY, int cropW, int cropH, int[] pix) {
			texIndex = tidx;
			crop = new int[4];
			// The region of the texture corresponding to the glyph is
			// surrounded by a
			// 1-pixel wide border to avoid artifacts due to bilinear sampling.
			// This is
