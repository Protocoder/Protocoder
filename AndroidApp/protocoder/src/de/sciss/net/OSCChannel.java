...
/**
 * A collection of common constants and methods that apply to all kinds of OSC
 * communicators. Formely being abstract superclass
 * <code>AbstractOSCCommunicator</code> of <code>OSCTransmitter</code> and
 * <code>OSCReceiver</code>, the functionality has been reduced and converted to
 * an interface as of v0.30. </P>
 * <P>
 * The method for getting a socket's <code>DatagramChannel</code> has been
 * removed in the course of TCP integration, also with keeping in mind the
 * possible future integration of other channel types such as
 * <code>FileChannel</code> or <code>PipeChannel</code>.
 * 
...
public interface OSCChannel {
	 ...
	 * HREF="http://en.wikipedia.org/wiki/User_Datagram_Protocol"
	 * >en.wikipedia.org/wiki/User_Datagram_Protocol</A> for explanation
	 */
	...
	/**
	 * The default buffer size (in bytes) and maximum OSC packet size (8K at the
	 * moment).
	 */
	...
	/**
	 * Queries the communicator's local socket address. You can determine the
	...
	/**
	 * Adjusts the buffer size for OSC messages. This is the maximum size an OSC
	 * packet (bundle or message) can grow to.
	 * 
	 * @param size
	...
	/**
	 * Queries the buffer size used for coding or decoding OSC messages. This is
	 * the maximum size an OSC packet (bundle or message) can grow to.
	 * 
	...
	/**
	 * Changes the way processed OSC messages are printed to the standard err
	 * console. By default messages are not printed.
	 * 
	 * @param mode
	...
	/**
	 * Disposes the resources associated with the OSC communicator. The object
