 ...
 * is accomplished. For example, by default, when an OSC message is assembled
 * for transmission, the encoder will translate a <code>java.lang.Integer</code>
 * argument into a four byte integer with typetag <code>'i'</code>. Or when a
 * received message is being decoded, finding an atom typetagged
 * <code>'f'</code>, the decoder will create a <code>java.lang.Float</code> out
 * of it.
 * <p>
 * This example sounds trivial, but the codec is also able to handle type
 * conversions. For instance, in the strict OSC 1.0 specification, only 32bit
 * numeric atoms are defined (<code>'i'</code> and <code>'f'</code>). A codec
 * with mode <code>MODE_STRICT_V1</code> will reject a
 * <code>java.lang.Double</code> in the encoding process and not be able to
 * decode a typetag <code>'d'</code>. A codec with mode <code>MODE_MODEST</code>
 * automatically breaks down everything the 32bit, so a
 * <code>java.lang.Double</code> gets encoded as 32bit <code>'f'</code> and a
 * received atom tagged <code>'d'</code> becomes a <code>java.lang.Float</code>.
 * Other configurations exist.
 * <p>
 * Another important function of the codec is to specify the charset encoding of
 * strings, something that was overseen in the OSC 1.0 spec. By default,
 * <code>UTF-8</code> is used so all special characters can be safely encoded.
 * <p>
 * Last but not least, using the <code>putDecoder</code> and
 * <code>putEncoder</code> methods, the codec can be extended to support
 * additional Java classes or OSC typetags, without the need to subclass
 * <code>OSCPacketCodec</code>.
 * 
...
public class OSCPacketCodec {
	 ...
	 * <code>java.lang.Float</code>, <code>java.lang.String</code>, and
	 * <code>byte[]</code>. Decoder only accepts <code>'i'</code>,
	 * <code>'f'</code>, <code>'s'</code>, and <code>'b'</code>. Note that
	 * <code>byte[]</code> is used to represents blobs ( <code>'b'</code>).
	 */
	 ...
	 * <code>de.sciss.net.OSCPacket</code> as a blob <code>'b'</code>. The
	 * decoder decodes <code>'h'</code> into <code>java.lang.Integer</code>,
	 * <code>'d'</code> into <code>java.lang.Float</code>, and <code>'S'</code>
	 * (Symbol) into <code>java.lang.String</code>.
	 */
	public static final int MODE_MODEST = MODE_READ_DOUBLE_AS_FLOAT | MODE_READ_LONG_AS_INTEGER
			| MODE_WRITE_DOUBLE_AS_FLOAT | MODE_WRITE_LONG_AS_INTEGER | MODE_READ_SYMBOL_AS_STRING
			| MODE_WRITE_PACKET_AS_BLOB;
	/**
	 * Support mode: like <code>MODE_MODEST</code>, that is, it will downgrade
	 * to 32bit in the encoding process, but decoding leaves 64bit values
	 * intact, so <code>'h'</code> becomes <code>java.lang.Long</code>, and
	 * <code>'d'</code> into <code>java.lang.Double</code>.
	 */
	public static final int MODE_GRACEFUL = MODE_READ_DOUBLE | MODE_READ_LONG | MODE_WRITE_DOUBLE_AS_FLOAT
			| MODE_WRITE_LONG_AS_INTEGER | MODE_READ_SYMBOL_AS_STRING | MODE_WRITE_PACKET_AS_BLOB;
	/**
	 * Support mode: like <code>MODE_STRICT_V1</code>, but with additional 64bit
	 * support, that is a mutual mapping between <code>'h'</code> &lt;--&gt;
	 * <code>java.lang.Long</code>, and <code>'d'</code> &lt;--&gt;
	 * <code>java.lang.Double</code>. Also, <code>'S'</code> (Symbol) is decoded
	 * into <code>java.lang.String</code>, and
	 * <code>de.sciss.net.OSCPacket</code> is encoded as a blob <code>'b'</code>
	 * .
	 */
	public static final int MODE_FAT_V1 = MODE_READ_DOUBLE | MODE_READ_LONG | MODE_WRITE_DOUBLE | MODE_WRITE_LONG
			| MODE_READ_SYMBOL_AS_STRING | MODE_WRITE_PACKET_AS_BLOB;
	 ...
	 * <code>UTF-8</code> encoding. Note that since a codec and be shared
	 * between <code>OSCServer</code> or <code>OSCClient</code> instances,
	 * usually you will just want to call <code>getDefaultCodec</code>!
	 * 
	 * @see #MODE_GRACEFUL
	 * @see #getDefaultCodec()
	 */
	...
	/**
	 * Creates a new codec with a given support mode and <code>UTF-8</code>
	 * encoding.
	 * 
	 * @param mode
	 *            the support mode flag field to use
	 */
	...
	/**
	 * Creates a new codec with a given support mode and a given charset for
	 * string encoding.
	 * 
	 * @param mode
	 *            the support mode flag mask to use
	 * @param charset
	 *            the name of the charset to use for string coding and decoding,
	 *            like <code>&quot;UTF-8&quot;</code>,
	...
	/**
	 * Queries the standard codec which is used in all implicit client and
	 * server creations. This codec adheres to the <code>MODE_GRACEFUL</code>
	 * scheme and uses <code>UTF-8</code> string encoding.
	 * <p>
	 * Note that although it is not recommended, it is possible to modify the
	 * returned codec. That means that upon your application launch, you could
	 * query the default codec and switch its behaviour, e.g. change the string
	 * charset, so all successive operations with the default codec will be
	 * subject to those customizations.
	 * 
	 ...
	 * 
	 * @param charsetName
	 *            the name of the charset, e.g. <code>&quot;UTF-8&quot;</code>,
	 *            <code>&quot;ISO-8859-1&quot;</code> etc.
	 * @see java.nio.charset.Charset
	 */
	...
	/**
	 * Registers an atomic decoder with the packet codec. This decoder is called
	 * whenever an OSC message with the given typetag is encountered.
	 * 
	 * @param typeTag
	 *            the typetag which is to be decoded with the new
	 *            <code>Atom</code>. <code>typeTag</code> must be in the ASCII
	...
	/**
	 * Registers an atomic encoder with the packet codec. This encoder is called
	 * whenever an OSC message to be assembled contains an argument of the given
	 * Java class.
	 * 
	 * @param javaClass
	 *            the class for which the encoder is responsible
	 * @param a
	 *            the encoder to use
	 * 
	 * @see OSCPacketCodec.Atom
	...
	/**
	 * Adjusts the support mode for type tag handling. Usually you specify the
	 * mode directly in the instantiation of <code>OSCPacketCodec</code>, but
	 * you can change it later using this method.
	 * 
	 * @param mode
	 *            the new mode to use. A flag field combination of
	 *            <code>MODE_READ_DOUBLE</code> or
	 *            <code>MODE_READ_DOUBLE_AS_FLOAT</code> etc., or a ready made
	 *            combination such as <code>MODE_FAT_V1</code>.
	 * 
	...
	/**
	 * Creates a new packet decoded from the ByteBuffer. This method tries to
	 * read a null terminated string at the beginning of the provided buffer. If
	 * it equals the bundle identifier, the <code>decode</code> of
	 * <code>OSCBundle</code> is called (which may recursively decode nested
	 * bundles), otherwise the one from <code>OSCMessage</code>.
	 * 
	 * @param b
	 *            <code>ByteBuffer</code> pointing right at the beginning of the
	 *            packet. the buffer's limited should be set appropriately to
	 *            allow the complete packet to be read. when the method returns,
	 *            the buffer's position is right after the end of the packet.
	 * 
	 * @return new decoded OSC packet
	 * 
	 * @throws IOException
	 *             in case some of the reading or decoding procedures failed.
	 * @throws BufferUnderflowException
	 *             in case of a parsing error that causes the method to read
	 *             past the buffer limit
	 * @throws IllegalArgumentException
	 *             occurs in some cases of buffer underflow
	 */
	 ...
	 * <code>ByteBuffer</code>, beginning at the buffer's current position. To
	 * write the encoded packet, you will typically call <code>flip()</code> on
	 * the buffer, then <code>write()</code> on the channel.
	 * 
	 * @param b
	 *            <code>ByteBuffer</code> pointing right at the beginning of the
	 *            osc packet. buffer position will be right after the end of the
	 *            packet when the method returns.
	 * 
	 * @throws IOException
	 ...
	 * 
	 * @return the size of the packet in bytes, including the initial osc
	 *         command and aligned to 4-byte boundary. this is the amount of
	 *         bytes written by the <code>encode</code> method.
	 * 
	 * @throws IOException
	...
	protected int getBundleSize(OSCBundle bndl) throws IOException {
		synchronized (bndl.collPackets) {
			int result = bndlIdentifier.length + 8 + (bndl.collPackets.size() << 2); // name,
			// timetag,
			// size
			// of
			// each
	 ...
	 * 
	 * @throws IOException
	 *             if the message contains invalid arguments
	 */
	...
	protected OSCBundle decodeBundle(ByteBuffer b) throws IOException {
		...
		try {
			...
			return bndl;
		} catch (IllegalArgumentException e1) { // throws by b.limit if bundle
			// size is corrupted
	...
	/**
	 * Creates a new message with arguments decoded from the ByteBuffer. Usually
	 * you call <code>decode</code> from the <code>OSCPacket</code> superclass
	 * which will invoke this method of it finds an OSC message.
	 * 
	 * @param b
	 *            ByteBuffer pointing right at the beginning of the type
	 *            declaration section of the OSC message, i.e. the name was
	 *            skipped before.
	 * 
	 * @return new OSC message representing the received message described by
	 *         the ByteBuffer.
	 * 
	 * @throws IOException
	 *             in case some of the reading or decoding procedures failed.
	...
	protected OSCMessage decodeMessage(String command, ByteBuffer b) throws IOException {
		...
		if (b.get() != 0x2C)
			throw new OSCException(OSCException.FORMAT, null);
	...
	/**
	 * Encodes the message onto the given <code>ByteBuffer</code>, beginning at
	 * the buffer's current position. To write the encoded message, you will
	 * typically call <code>flip()</code> on the buffer, then
	 * <code>write()</code> on the channel.
	 * 
	 * @param b
	 *            <code>ByteBuffer</code> pointing right at the beginning of the
	 *            osc packet. buffer position will be right after the end of the
	 *            message when the method returns.
	 * 
	 * @throws IOException
	 *             in case some of the writing procedures failed (buffer
	 *             overflow, illegal arguments).
	 */
	protected void encodeMessage(OSCMessage msg, ByteBuffer b) throws BufferOverflowException, IOException {
		...
		b.position(b.position() + ((numArgs + 5) & ~3)); // comma + numArgs +
		// zero + align
		try {
			for (int i = 0; i < numArgs; i++) {
				o = msg.getArg(i);
	 ...
	 * 
	 * @param b
	 *            buffer to read from. position and limit must be set
	 *            appropriately. new position will be right after the
	...
	/**
	 * Adds as many zero padding bytes as necessary to stop on a 4 byte
	 * alignment. if the buffer position is already on a 4 byte alignment when
	 * calling this function, another 4 zero padding bytes are added. buffer
	 * position will be on the new aligned boundary when return from this method
	 * 
	 * @param b
	 *            the buffer to pad
	 * 
	 ...
	 * Adds as many zero padding bytes as necessary to stop on a 4 byte
	 * alignment. if the buffer position is already on a 4 byte alignment when
	 * calling this function, this method does nothing.
	 * 
	 * @param b
	 *            the buffer to align
	 * 
	...
	/**
	 * Advances in the buffer as long there are non-zero bytes, then advance to
	...
	/**
	 * Advances the current buffer position to an integer of four bytes. The
	 * position is not altered if it is already aligned to a four byte boundary.
	 * 
	 * @param b
	 *            the buffer to advance
	 * 
	...
	/**
	 * The <code>Atom</code> class represents a combination of an encoder and
	 * decoder of a Java respectively OSC atom. While typically an
	 * <code>Atom</code> does a one-to-one mapping between a single Java class
	 * and a single OSC type tag, other mappings are possible, such as different
	 * type tags for the same Java class, or decoding the same typetag into
	 * different Java classes.
	 * <p>
	 * An example of the <B>first case</B> would be a colour atom: The
	 * <code>decodeAtom</code> method would require a <code>'r'</code> typetag
	 * and return a <code>java.awt.Color</code> object, with a body similar to
	 * this:
	 * 
	 * <PRE>
	 ...
	 * </PRE>
	 * 
	 * The <code>encodeAtom</code> and <code>getTypeTag</code> methods would
	 * require its argument to be a <code>java.awt.Color</code>,
	 ...
	 * </pre>
	 * <p>
	 * And example of the <B>latter case</B> (one-to-many mapping) would be a
	 * codec for the <code>'T'</code> (&quot;true&quot;) and <code>'F'</code>
	 * (&quot;false&quot;) typetags. This codec would be registered once as an
	 * encoder, using <code>putEncoder( Boolean.class, myAtomCodec )</code>, and
	 * twice as a decoder, using
	 * <code>putDecoder( (byte) 'F', myAtomCodec )</code> and
	 * <code>putDecoder( (byte) 'T', myAtomCodec )</code>. The codec's
	 * <code>getAtomSize</code> method would return <code>0</code>,
	 * <code>getTypeTag</code> would return
	 * 
	 * <pre>
	 ...
	 * @see OSCPacketCodec#putEncoder(java.lang.Class,
	 *      de.sciss.net.OSCPacketCodec.Atom )
	 */
	...
	// implements Atom
		...
		public void encodeAtom(Object o, ByteBuffer tb, ByteBuffer db) throws IOException {
			tb.put((byte) 0x73); // 's'
			final String s = (String) o; // cassting seems tp be faster tan
			// toString()!
			db.put(s.getBytes(charsetName)); // faster than using Charset or
			// CharsetEncoder
