 ...
 * OSC communication from the perspective of a client. It simplifies the need to
 * use several objects by uniting their functionality. </P>
 * <P>
 * In the following example, a client for UDP to SuperCollider server (scsynth)
 * on the local machine is created. The client starts a synth by sending a
 * <code>/s_new</code> message, and stops the synth by sending a delayed a
 * <code>/n_set</code> message. It waits for the synth to die which is
 * recognized by an incoming <code>/n_end</code> message from scsynth after
 * we've registered using a <code>/notify</code> command.
 * 
 * <pre>
...
public class OSCClient implements OSCBidi {
	...
	/**
	 * Creates a new instance of an <code>OSCClient</code>, using default codec
	...
	/**
	 * Creates a new instance of an <code>OSCClient</code>, using a specific
	 ...
	 * client listens and from which it sends), it does not determine the remote
	 * sockets from which messages can be received and to which messages are
	 * sent. The target socket can be set using the <code>setTarget</code>
	 * method!
	 * 
	 * @param protocol
	 *            the protocol to use, currently either <code>UDP</code> or
	...
	/**
	 * Creates a new instance of an <code>OSCClient</code>, using default codec
	 * and a specific transport protocol and local socket address. Note that
	 * <code>localAdress</code> specifies the local socket (at which the
	 * receiver listens and from which the transmitter sends), it does not
	 * determine the remote sockets to which the client connects. To specify the
	 * remote socket, use the <code>setTarget</code> method!
	 * <P>
	 * 
	 * @param protocol
	 *            the protocol to use, currently either <code>UDP</code> or
	...
	/**
	 * Creates a new instance of an <code>OSCClient</code>, using a given codec,
	 * a specific transport protocol and local socket address. Note that
	 * <code>localAdress</code> specifies the local socket (at which the
	 * receiver listens and from which the transmitter sends), it does not
	 * determine the remote sockets to which the client connects. To specify the
	 * remote socket, use the <code>setTarget</code> method!
	...
	/**
	 * Queries the client side socket address. This is the address from which
	 * the client sends and at which it listens for replies. You can determine
	 * the host and port from the returned address by calling
	 * <code>getHostName()</code> (or for the IP
	 * <code>getAddress().getHostAddress()</code>) and <code>getPort()</code>.
	 * <p>
	 * Note that if the client is bound to the accept-any IP
	...
	/**
	 * Specifies the client's target address, that is the address of the server
	 * to talk to. You should call this method only once and you must call it
	 * before starting the client or sending messages.
	 * 
	 * @param target
	 *            the address of the server. Usually you construct an
	 *            appropriate <code>InetSocketAddress</code>
	 * 
	...
	/**
	 * Initializes network channel (if necessary) and establishes connection for
	 * transports requiring connectivity (e.g. TCP). Do not call this method
	 * when the client is already connected. Note that <code>start</code>
	 * implicitly calls <code>connect</code> if necessary, so usually you will
	 * not need to call <code>connect</code> yourself.
	 * 
	 * @throws IOException
	 ...
	 *             transport is TCP and the client was stopped before (unable to
	 *             revive).
	 * 
	 * @see #isConnected()
	 * @see #start()
	 ...
	 *         otherwise. For transports that do not use connectivity (e.g. UDP)
	 *         this returns <code>false</code>, if the underlying
	 ...
	 * Make sure that the client's target has been specified before by calling
	 * <code>setTarget()</code>
	 * 
	 * @param p
	 *            the packet to send
	 ...
	 *             or network error occurs, for example if a TCP client was not
	 *             connected before.
	 * @throws NullPointerException
	 *             for a UDP client if the target has not been specified
	 * 
	 ...
	 * call this both when the client is active or inactive.
	 * 
	 * @param listener
	 ...
	 * requires connectivity (e.g. TCP) and the channel is not yet connected. It
	 * then tells the underlying OSC receiver to start listening.
	 * 
	 * @throws IOException
	 *             if a networking error occurs. Possible reasons: - the
	 *             underlying network channel had been closed by the server. -
	 ...
	 *          clients after the server has closed the connection. Also it's
	 *          not possible to start a TCP client more than once. This might be
	 ...
	 * calling its <code>start()</code> method and deactivated by calling
	 * <code>stop()</code>.
	 * 
	 * @return <code>true</code> if the client is active (connected and
	 *         listening), <code>false</code> otherwise.
	 * 
	 * @see #start()
	 * @see #stop()
	 */
	...
	/**
	 * Adjusts the buffer size for OSC messages (both for sending and
	 * receiving). This is the maximum size an OSC packet (bundle or message)
	 * can grow to. The initial buffer size is <code>DEFAULTBUFSIZE</code>. Do
	 * not call this method while the client is active!
	 * 
	 * @param size
	 *            the new size in bytes.
	 * 
	 * @throws IllegalStateException
	 *             if trying to change the buffer size while the client is
	 *             active (listening).
	 * 
	 * @see #isActive()
	 * @see #getBufferSize()
	...
	/**
	 * Queries the buffer size used for sending and receiving OSC messages. This
	 * is the maximum size an OSC packet (bundle or message) can grow to. The
	 * initial buffer size is <code>DEFAULTBUFSIZE</code>.
	 * 
	...
	/**
	 * Changes the way incoming and outgoing OSC messages are printed to the
	 * standard err console. By default messages are not printed.
	 * 
	 * @param mode
	 *            one of <code>kDumpOff</code> (don't dump, default),
	 *            <code>kDumpText</code> (dump human readable string),
	 *            <code>kDumpHex</code> (hexdump), or <code>kDumpBoth</code>
	 *            (both text and hex)
	 * @param stream
	 *            the stream to print on, or <code>null</code> which is
	 *            shorthand for <code>System.err</code>
	 * 
	...
	/**
	 * Destroys the client and frees resources associated with it. This
	 * automatically stops the client and closes the networking channel. Do not
	 * use this client instance any more after calling <code>dispose.</code>
	 */
	public void dispose() {
		rcv.dispose();
		trns.dispose();
