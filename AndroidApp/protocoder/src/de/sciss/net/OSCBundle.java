...
/**
 * Implementation of the OSC-Bundle which assembles several OSC-Packets under
 * the same timetag. See <A
 * HREF="http://opensoundcontrol.org/spec-1_0">opensoundcontrol.org/spec-1_0</A>
 * for the specification of the bundle format.
 * <p>
 * The bundle time can be calculated in different ways: specifying a
 * <code>long</code> which represents the milliseconds since 1 jan 1970 as
 * returned by <code>System.currentTimeMillis()</code>, produces the required
 * network time tag as required by the OSC specification. Alternatively, using a
 * <code>float</code> for a relative time offset in seconds can be used when
 * running SuperCollider in offline mode. Third, there is a version that
 * calculates a sample accurate time tag. However, scsynth doesn't process
 * bundles with this accuracy, so it's kind of useless.
 * <p>
 * To assemble a bundle, you create a new instance of <code>OSCBundle</code>,
 * call <code>addPacket</code> one or several times, then write the contents of
 * the bundle to a <code>ByteBuffer</code> using the method <code>encode</code>.
 * The byte buffer can then be written to a <code>DatagramChannel</code>. Here
 * is an example:
 * 
 * <pre>
 * OSCBundle bndl;
 ...
 * 	bndl.addPacket(new OSCMessage(&quot;/s_new&quot;, new Object[] { &quot;default&quot;, new Integer(1001), new Integer(1),
 * 			new Integer(0), &quot;out&quot;, new Integer(0), &quot;freq&quot;, new Float(666), &quot;amp&quot;, new Float(0.1f) }));
 * 	bndl.encode(buf);
 * 	buf.flip();
 * 	dch.send(buf, addr);
 * 	buf.clear();
