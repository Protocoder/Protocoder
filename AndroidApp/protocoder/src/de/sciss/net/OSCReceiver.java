...
/**
 * An <code>OSCReceiver</code> manages reception of incoming OSC messages. A
 * receiver can be either <B>revivable</B> or <B>non-revivable</B>.
 * <UL>
 ...
 * TCP)). When the channel is closed, the receiver cannot be restarted. The
 * network channel is closed for example when a TCP server shuts down, but also
 * when trying to connect to a TCP server that is not yet reachable.</LI>
 * <LI>It is therefore recommended to use revivable receivers. A revivable
 * receiver is created through one of the <code>newUsing</code> constructors
 * that takes a protocol and address argument. A revivable receiver can be
 * restarted because it recreates the network channel if necessary.</LI>
 * </UL>
 * <P>
 * The receiver launches a listening <code>Thread</code> when
 * <code>startListening</code> is called.
 * <p>
 * The <code>OSCReceiver</code> has methods for registering and unregistering
 ...
 * <code>OSCReceiver</code>. Also note that as of v0.3, <code>OSCReceiver</code>
 * is abstract, which renders direct instantiation impossible. <B>To update old
 * code,</B> occurrences of <code>new OSCReceiver()</code> must be replaced by
 * one of the <code>OSCReceiver.newUsing</code> methods! The &quot;filter&quot;
 * functionality of NetUtil 0.2 is now implied by calling
 * <code>setTarget( SocketAddress )</code>.
 * <P>
 * Here is an example that also demonstrates message sending using an instance
 * of <code>OSCTransmitter</code>:
 * 
 * <pre>
 ...
 * </pre>
 * 
 * Note that the datagram channel needs to be bound to a valid reachable
 * address, because <code>stopListening</code> will be sending a terminating
 * message to this channel. You can bind the channel using
 ...
 *                  must not be carried out in the OSC receiver thread.
