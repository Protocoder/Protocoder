 ...
 *     // transmitters cannot be revived. to create revivable channels on the same
 *     // socket,
 *     // you must use one of the newUsing methods that take an IP address and/or
 *     // port
 *     // number.
 ...
 * 		System.out.println(&quot;scsynth is running. contains &quot; + msg.getArg(1) + &quot; unit generators, &quot;
 * 			+ msg.getArg(2) + &quot; synths, &quot; + msg.getArg(3) + &quot; groups, &quot; + msg.getArg(4) + &quot; synth defs.\n&quot;
...
public abstract class OSCReceiver implements OSCChannel, Runnable {
    ...
    /**
     * Creates a new instance of a revivable <code>OSCReceiver</code>, using default codec and a specific transport
     * protocol. It picks an arbitrary free port and uses the local machine's IP. To determine the resulting port, you
     * can use <code>getLocalAddress</code> afterwards.
     * <P>
     ...
     * 
     * @param protocol
     *            the protocol to use, currently either <code>UDP</code> or <code>TCP</code>
    ...
    /**
     * Creates a new instance of a revivable <code>OSCReceiver</code>, using a specific codec and transport protocol. It
     * picks an arbitrary free port and uses the local machine's IP. To determine the resulting port, you can use
    ...
    /**
     * Creates a new instance of a revivable <code>OSCReceiver</code>, using default codec and a specific transport
     * protocol and port. It uses the local machine's IP. Note that the <code>port</code> specifies the local socket (at
     * which the receiver listens), it does not determine the remote sockets from which messages can be received. If you
     * want to filter out a particular remote (or target) socket, this can be done using the <code>setTarget</code>
     * method!
     * <P>
     * <B>TCP</B> receivers are required to be connected to one particular target, so <code>setTarget</code> is must be
     * called prior to <code>connect</code> or <code>startListening</code>!
     ...
     *            the protocol to use, currently either <code>UDP</code> or <code>TCP</code>
     * @param port
     *            the port number for the OSC socket, or <code>0</code> to use an arbitrary free port
     * @return the newly created receiver
     * 
     * @throws IOException
     *             if a networking error occurs while creating the socket
    ...
    /**
     * Creates a new instance of a revivable <code>OSCReceiver</code>, using a specific codec and transport protocol and
     * port. It uses the local machine's IP. Note that the <code>port</code> specifies the local socket (at which the
     * receiver listens), it does not determine the remote sockets from which messages can be received. If you want to
     * filter out a particular remote (or target) socket, this can be done using the <code>setTarget</code> method!
     * <P>
     * <B>TCP</B> receivers are required to be connected to one particular target, so <code>setTarget</code> is must be
     * called prior to <code>connect</code> or <code>startListening</code>!
     * 
    ...
    /**
     * Creates a new instance of a revivable <code>OSCReceiver</code>, using default codec and a specific transport
     * protocol and port. It uses the local machine's IP or the &quot;loopback&quot; address. Note that the
     * <code>port</code> specifies the local socket (at which the receiver listens), it does not determine the remote
     * sockets from which messages can be received. If you want to filter out a particular remote (or target) socket,
     * this can be done using the <code>setTarget</code> method!
     * <P>
     * <B>TCP</B> receivers are required to be connected to one particular target, so <code>setTarget</code> is must be
     * called prior to <code>connect</code> or <code>startListening</code>!
     * 
     ...
     *            the port number for the OSC socket, or <code>0</code> to use an arbitrary free port
     * @param loopBack
     *            if <code>true</code>, the &quot;loopback&quot; address ( <code>&quot;127.0.0.1&quot;</code>) is used
     *            which limits communication to the local machine. If <code>false</code>, the special IP
     *            <code>"0.0.0.0"</code> is used which means messages from any IP as well as from the loopback are
     *            accepted
     * 
     * @return the newly created receiver
     * 
    ...
    /**
     * Creates a new instance of a revivable <code>OSCReceiver</code>, using a specific codec and transport protocol and
     * port. It uses the local machine's IP or the &quot;loopback&quot; address. Note that the <code>port</code>
     * specifies the local socket (at which the receiver listens), it does not determine the remote sockets from which
     * messages can be received. If you want to filter out a particular remote (or target) socket, this can be done
    ...
    /**
     * Creates a new instance of a revivable <code>OSCReceiver</code>, using default codec and a specific transport
     * protocol and local socket address. Note that <code>localAdress</code> specifies the local socket (at which the
     * receiver listens), it does not determine the remote sockets from which messages can be received. If you want to
     * filter out a particular remote (or target) socket, this can be done using the <code>setTarget</code> method!
     * <P>
     * <B>TCP</B> receivers are required to be connected to one particular target, so <code>setTarget</code> is must be
     ...
     *            the protocol to use, currently either <code>UDP</code> or <code>TCP</code>
     * @param localAddress
     *            a valid address to use for the OSC socket. If the port is <code>0</code>, an arbitrary free port is
     *            picked when the receiver is started. (you can find out the actual port in this case by calling
     *            <code>getLocalAddress()</code> after the receiver was started).
     * 
     * @return the newly created receiver
     * 
     * @throws IOException
    ...
    /**
     * Creates a new instance of a revivable <code>OSCReceiver</code>, using a specific codec and transport protocol and
     * local socket address. Note that the <code>port</code> specifies the local socket (at which the receiver listens),
    ...
    /**
     * Creates a new instance of a non-revivable <code>OSCReceiver</code>, using default codec and UDP transport on a
     ...
     * the receiver listens), it does not determine the remote sockets from which messages can be received. If you want
    ...
    /**
     * Creates a new instance of a non-revivable <code>OSCReceiver</code>, using a specific codec and UDP transport on a
     * given channel. The caller should ensure that the provided channel's socket was bound to a valid address (using
     * <code>dch.socket().bind( SocketAddress )</code>). Note that <code>dch</code> specifies the local socket (at which
     * the receiver listens), it does not determine the remote sockets from which messages can be received. If you want
     * to filter out a particular remote (or target) socket, this can be done using the <code>setTarget</code> method!
    ...
    /**
     * Creates a new instance of a non-revivable <code>OSCReceiver</code>, using default codec and TCP transport on a
     * given channel. The caller should ensure that the provided channel's socket was bound to a valid address (using
     ...
     * socket (at which the receiver listens), it does not determine the remote sockets from which messages can be
    ...
    /**
     * Creates a new instance of a non-revivable <code>OSCReceiver</code>, using a specific codec and TCP transport on a
     * given channel. The caller should ensure that the provided channel's socket was bound to a valid address (using
     * <code>sch.socket().bind( SocketAddress )</code>). Furthermore, the channel must be connected (using
     * <code>connect()</code>) before being able to receive messages. Note that <code>sch</code> specifies the local
     * socket (at which the receiver listens), it does not determine the remote sockets from which messages can be
    ...
    /**
     * Queries the receiver's local socket address. You can determine the host and port from the returned address by
     * calling <code>getHostName()</code> (or for the IP <code>getAddress().getHostAddress()</code>) and
     * <code>getPort()</code>. This port number may be <code>0</code> if the receiver was called with an unspecified
     * port and has not yet been started. In this case, to determine the port actually used, call this method after the
     * receiver has been started.
     * <p>
     * Note that if the receiver is bound to the accept-any IP <code>"0.0.0.0"</code>, which happens for example when
     * calling <code>newUsing( &lt;protocol&gt;, 0, false )</code>, the returned IP will be the localhost's IP, so you
     * can patch the result directly into any <code>setTarget</code> call.
     * 
     ...
     * 
     * @throws IOException
     *             when an error occurs while establishing the channel connection. In that case, no thread has been
     *             started and hence stopListening() needn't be called
     * 
     * @throws IllegalStateException
     *             when trying to call this method from within the OSC receiver thread (which would obviously cause a
     *             loop)
     */
     ...
     * <P>
     * When a <B>UDP</B> transmitter is created without an explicit <code>DatagramChannel</code> &ndash; say by calling
     ...
     * 
     * @throws IOException
     *             if a networking error occurs. Possible reasons: - the underlying network channel had been closed by
     *             the server. - the transport is TCP and the server is not available.
     * 
     * @see #isConnected()
     * @see #startListening()
     * @throws IOException
     ...
     * 
     * @return <code>true</code> if the receiver is connected, <code>false</code> otherwise. For transports that do not
	...
	public void run() {
		...
		listen: while (isListening) {
			...
			if (isListening) {
			    ...
			    System.err.println("OSCReceiver.run : " + e1.getClass().getName() + " : "
	...
	public void run() {
		...
		listen: while (isListening) {
			...
			if (isListening) {
			    ...
			    System.err.println("OSCReceiver.run : " + e2.getClass().getName() + " : "
