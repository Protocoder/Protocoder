...
public abstract class NetUtilTest {
	...
	/**
	 * Tests the client functionality on a given protocol. This assumes
	 * SuperCollider server (scsynth) is running on the local machine, listening
	 * at the given protocol and port 57110.
	 * 
	 * @param protocol
	...
	/**
	 * Tests the performance of OSCMessage encoding and decoding. A cyclic
	 * random list of messages get decoded and encoded for five seconds, the
	 * number of codec operations during this interval is printed.
	 * 
	 * Benchmarks for MacBook Pro 2.0 GHz, Mac OS X 10.4.8, 1.5.0_07: NetUtil
	 * 0.33 (build 07-May-07) vs. Illposed JavaOSC (20060402): encoding :
	 * NetUtil roughly 220% faster decoding : Illposed roughly 50% faster
	 * 
	 * NetUtil 0.32 vs. 0.33 are very similar, decoding is a few percent faster
	 * in v0.33
	 */
	...
	/**
	 * Creates two receivers and two transmitters, one of each being restricted
	 * to loopback. Sends from each transmitter to each receiver. The expected
	 * result is that all messages arrive except those sent from the local host
