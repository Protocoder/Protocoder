...
public abstract class NetUtilTest {
    ...
    /**
     * Tests the client functionality on a given protocol. This assumes SuperCollider server (scsynth) is running on the
     * local machine, listening at the given protocol and port 57110.
     * 
     * @param protocol
	...
	for (int i = 0; i < 4; i++) {
	    ...
	    bndl1.addPacket(new OSCMessage("/n_set", new Object[] { nodeID, "freq",
		    new Float(Math.pow(2, (float) i / 6) * 441) }));
    ...
    /**
     * Tests the server functionality on a given protocol. This opens a server listening at port 0x5454. Recognized
     * messages are <code>/pause</code>, <code>/quit</code>, <code>/dumpOSC</code>. See <code>NetUtil_Tests.rtf</code>
     * for a way to check the server.
     * 
     * @param protocol
     *            <code>UDP</code> or <code>TCP</code>
     */
    ...
    /**
     * Tests the performance of OSCMessage encoding and decoding. A cyclic random list of messages get decoded and
     * encoded for five seconds, the number of codec operations during this interval is printed.
     * 
     * Benchmarks for MacBook Pro 2.0 GHz, Mac OS X 10.4.8, 1.5.0_07: NetUtil 0.33 (build 07-May-07) vs. Illposed
     * JavaOSC (20060402): encoding : NetUtil roughly 220% faster decoding : Illposed roughly 50% faster
     * 
     * NetUtil 0.32 vs. 0.33 are very similar, decoding is a few percent faster in v0.33
     */
    ...
    /**
     * Creates two receivers and two transmitters, one of each being restricted to loopback. Sends from each transmitter
     * to each receiver. The expected result is that all messages arrive except those sent from the local host
     * transmitter to loopback receiver (trns2 to rcv1, i.e. "five", "six").
     */
	...
	for (int i = 0; i < 1; i++) { // note: tcp requires a server socket
		...
		try {
		    ...
		    final InetSocketAddress targetAddr = trnsLoop ? new InetSocketAddress("127.0.0.1", targetPort)
	...
	for (int i = 0; i < protos.length; i++) {
		...
		try {
		    ...
		    final InetSocketAddress targetAddr = clientLoop ? new InetSocketAddress("127.0.0.1", targetPort)
			    : new InetSocketAddress(InetAddress.getLocalHost(), targetPort);
