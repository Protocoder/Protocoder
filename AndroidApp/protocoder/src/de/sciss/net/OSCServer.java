...
/**
 * This class dynamically groups together a transmitters and receivers, allowing
 * bidirectional OSC communication from the perspective of a server. It
 * simplifies the need to use several objects by uniting their functionality,
 * and by automatically establishing child connections.
 * <P>
 * In <code>UDP</code> mode, simply one receiver and transmitter are handling
 * all the communication. In <code>TCP</code> mode, a
 * <code>ServerSocketChannel</code> is set up to wait for incoming connection
 * requests. Requests are satisfied by opening a new receiver and transmitter
 * for each connection.
 * <P>
 * In the following example, a simple TCP server is created that accepts
 * connections at port 0x5454. The connections understand the OSC commands
 * <code>/pause</code> (disconnect the server for a few seconds),
 * <code>/quit</code> (quit the server), and <code>/dumpOSC</code> (turn on/off
 * printing of message traffic). Each incoming message is replied with a
