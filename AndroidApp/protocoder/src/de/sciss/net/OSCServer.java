...
public abstract class OSCServer implements OSCBidi {
    ...
    /**
     * Creates a new instance of an <code>OSCServer</code>, using default codec and a specific transport protocol. It
    ...
    /**
     * Creates a new instance of an <code>OSCServer</code>, using a specific codec and transport protocol. It picks an
    ...
    /**
     * Creates a new instance of an <code>OSCServer</code>, using default codec and a specific transport protocol and
     * port. It uses the local machine's IP.
     * <p>
     * Note that the <code>port</code> specifies the local socket (at which the server listens and from which it sends,
     * or in the case of TCP transport, from which it establishes client connections), it does not determine the remote
     * sockets. The address of a remote client communicating to this server is passed in the
     * <code>messageReceived</code> method of any registered <code>OSCListener</code>, and must be picked up and handed
     * in to the <code>send</code> method to reply back to the client!
     * 
     * @param protocol
     *            the protocol to use, currently either <code>UDP</code> or <code>TCP</code>
     * @param port
    ...
    /**
     * Creates a new instance of an <code>OSCServer</code>, using a specific codec and transport protocol and port. It
     ...
     *            <code>getLocalAddress()</code> after the server was started).
     * @return the newly created server
     * 
     * @throws IOException
     *             if a networking error occurs while creating the socket
    ...
    /**
     * Queries the server socket's address. This is the address at which the server accepts connections (when using TCP)
     * or receives and sends messages (when using UDP). You can determine the host and port from the returned address by
     * calling <code>getHostName()</code> (or for the IP <code>getAddress().getHostAddress()</code>) and
     * <code>getPort()</code>.
     * <p>
     * Note that if the server is bound to the accept-any IP <code>"0.0.0.0"</code>, which happens for example when
     ...
     * 
     * @throws IOException
     *             if a write error, OSC encoding error, buffer overflow error or network error occurs, if no client
     *             connection for the given address exists
     */
    ...
    /**
     * Destroys the server and frees resources associated with it. This automatically stops the server and closes the
     * networking channels. Do not use this server instance any more after calling <code>dispose.</code>
     */
    ...
    private static class TCPOSCServer extends OSCServer implements Runnable, OSCListener {
	...
	public void stop() throws IOException {
		...
		if (isListening) {
			...
			try {
			    ...
			    if ((thread != null) && thread.isAlive()) {
				try {
				    System.err.println("TCPServerBody.stopListening : rude task killing ("
					    + this.hashCode() + ")");
