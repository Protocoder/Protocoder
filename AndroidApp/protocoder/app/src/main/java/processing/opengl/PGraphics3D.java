...
public class PGraphics3D extends PGraphicsOpenGL {
	...
	static protected PShape loadShapeImpl(PGraphics pg, String filename, String ext) {
		...
		for (int i = 0; i < faces.size(); i++) {
			...
			if (mtlIdxCur != face.matIdx) {
				mtlIdxCur = PApplet.max(0, face.matIdx); // To make sure that at
															// least we get the
															// default material.
			...
			if (face.vertIdx.size() == 3) {
				child = createShapeImpl(pg.parent, TRIANGLES); // Face is a
																// triangle, so
																// using
																// appropriate
																// shape kind.
			} else if (face.vertIdx.size() == 4) {
				child = createShapeImpl(pg.parent, QUADS); // Face is a quad, so
															// using appropriate
															// shape kind.
			} else {
				child = createShapeImpl(pg.parent, POLYGON); // Face is a
																// general
																// polygon
			}
			...
			if (mtl.kdMap != null) {
				// If current material is textured, then tinting the texture
				// using the diffuse color.
	...
	static protected void parseOBJ(PApplet parent, BufferedReader reader, ArrayList<PVector> vertices,
			ArrayList<PVector> normals, ArrayList<PVector> textures, ArrayList<OBJFace> faces,
			...
			while ((line = reader.readLine()) != null) {
				...
				if (elements.length > 0) {
					if (elements[0].equals("v")) {
						// vertex
						PVector tempv = new PVector(Float.valueOf(elements[1]).floatValue(), Float.valueOf(elements[2])
								.floatValue(), Float.valueOf(elements[3]).floatValue());
						vertices.add(tempv);
						readv = true;
					} else if (elements[0].equals("vn")) {
						// normal
						PVector tempn = new PVector(Float.valueOf(elements[1]).floatValue(), Float.valueOf(elements[2])
								.floatValue(), Float.valueOf(elements[3]).floatValue());
						normals.add(tempn);
						readvn = true;
					} else if (elements[0].equals("vt")) {
						// uv, inverting v to take into account Processing's
						// invertex Y axis with
						// respect to OpenGL.
						PVector tempv = new PVector(Float.valueOf(elements[1]).floatValue(), 1 - Float.valueOf(
								elements[2]).floatValue());
						textures.add(tempv);
						readvt = true;
						...
						// applied on all subsequent faces).
						if (elements[1] != null) {
	...
	static protected void parseMTL(PApplet parent, BufferedReader reader, ArrayList<OBJMaterial> materials,
