...
class PriorityQSort extends PriorityQ {
	...
	boolean pqInit() {
		...
		/*
		 * Create an array of indirect pointers to the keys, so that we the
		 * handles we have returned are still valid.
		 */
		...
		for (piv = 0, i = p; i <= r; ++piv, ++i) {
			// indirect pointers: keep an index into the keys array, not a
			// direct pointer to its contents
			order[i] = piv;
		}

		/*
		 * Sort the indirect pointers in descending order, using randomized
		 * Quicksort
		 */
		stack[top].p = p;
		stack[top].r = r;
		...
		/*
		 * #ifndef NDEBUG p = order; r = p + size - 1; for (i = p; i < r; ++i) {
		 * Assertion.doAssert(LEQ( * * (i + 1), **i )); } #endif
		 */
	...
	int pqInsert(Object keyNew) {
		...
		if (++size >= max) {
			...
			max <<= 1;
			// pq->keys = (PQHeapKey *)memRealloc( pq->keys,(size_t)(pq->max *
			// sizeof( pq->keys[0] )));
