 ...
 * Each instance takes a network channel, either explictly specified by a
 * <code>DatagramChannel</code> (for UDP) or <code>SocketChannel</code> (for
 * TCP), or internally opened when a protocol type is specified.
 * <P>
 * Messages are send by invoking one of the <code>send</code> methods. For an
 * example, please refer to the <code>OSCReceiver</code> document.
 * <P>
 * <B>Note that as of v0.3,</B> you will most likely want to use preferably one
 * of <code>OSCClient</code> or <code>OSCServer</code> over
 * <code>OSCTransmitter</code>. Also note that as of v0.3,
 * <code>OSCTransmitter</code> is abstract, which renders direct instantiation
 * impossible. <B>To update old code,</B> occurences of
 * <code>new OSCTransmitter()</code> must be replaced by one of the
 * <code>OSCTransmitter.newUsing</code> methods!
 * 
 ...
 * @todo an explicit disconnect method might be useful (this is implicitly done
...
public abstract class OSCTransmitter implements OSCChannel {
	...
	/**
	 * Creates a new instance of an <code>OSCTransmitter</code>, using default
	 * codec and a specific transport protocol. It picks an arbitrary free port
	...
	/**
	 * Creates a new instance of an <code>OSCTransmitter</code>, using a
	...
	/**
	 * Creates a new instance of an <code>OSCTransmitter</code>, using default
	 * codec and a specific transport protocol and port. It uses the local
	 * machine's IP. Note that the <code>port</code> specifies the local socket,
	 * not the remote (or target) port. This can be set using the
	 ...
	 * <code>port</code> specifies the local socket, not the remote (or target)
	 ...
	 *            special IP <code>"0.0.0.0"</code> is used which means any
	 *            local host is picked
	 * 
	 * @return the newly created transmitter
	 * 
	 * @throws IOException
	...
	/**
	 * Creates a new instance of an <code>OSCTransmitter</code>, using default
	 * codec and a specific transport protocol and local socket address. Note
	 * that <code>localAddress</code> specifies the local socket, not the remote
	 * (or target) socket. This can be set using the <code>setTarget</code>
	 * method!
	 * 
	 * @param protocol
	 *            the protocol to use, currently either <code>UDP</code> or
	 ...
	 *            transmitter is connected. (you can find out the actual port in
	 *            this case by calling <code>getLocalAddress()</code> after the
	 ...
	 * <code>localAddress</code> specifies the local socket, not the remote (or
	...
	/**
	 * Creates a new instance of an <code>OSCTransmitter</code>, using default
	 * codec and UDP transport on a given channel. The caller should ensure that
	 * the provided channel's socket was bound to a valid address (using
	 * <code>dch.socket().bind( SocketAddress )</code>). Note that
	 * <code>dch</code> specifies the local socket, not the remote (or target)
	...
	/**
	 * Creates a new instance of an <code>OSCTransmitter</code>, using a
	 * specific codec and UDP transport on a given channel. The caller should
	...
	/**
	 * Creates a new instance of an <code>OSCTransmitter</code>, using default
	 * codec and TCP transport on a given channel. The caller should ensure that
	 * the provided channel's socket was bound to a valid address (using
	 * <code>sch.socket().bind( SocketAddress )</code>). Furthermore, the
	 * channel must be connected (using <code>connect()</code>) before being
	 * able to transmit messages. Note that <code>sch</code> specifies the local
	 * socket, not the remote (or target) socket. This can be set using the
	 * <code>setTarget</code> method!
	 * 
	 * @param sch
	...
	/**
	 * Creates a new instance of an <code>OSCTransmitter</code>, using a
	 * specific codec and TCP transport on a given channel. The caller should
	 * ensure that the provided channel's socket was bound to a valid address
	...
	/**
	 * Queries the transmitter's local socket address. You can determine the
	 * host and port from the returned address by calling
	 * <code>getHostName()</code> (or for the IP
	 * <code>getAddress().getHostAddress()</code>) and <code>getPort()</code>.
	 * This port number may be <code>0</code> if the transmitter was called with
	 * an unspecified port and has not yet been connected. In this case, to
	 * determine the port actually used, call this method after the transmitter
	 * has been connected.
	 * 
	...
	/**
	 * Specifies the transmitter's target address, that is the address of the
	 * remote side to talk to. You should call this method only once and you
	 * must call it before starting to send messages using the shortcut call
	 * <code>send( OSCPacket )</code>.
	 * 
	 * @param target
	 *            the address of the remote target socket. Usually you construct
	 *            an appropriate <code>InetSocketAddress</code>
	 * 
	 ...
	 * <code>OSCTransmitter.newUsing( &quot;udp&quot; )</code>, you are required
	 * to call <code>connect()</code> so that an actual
	 * <code>DatagramChannel</code> is created and bound. For a <B>UDP</B>
	 * transmitter which was created with an explicit
	 * <code>DatagramChannel</code>, this method does noting, so it is always
	 * safe to call <code>connect()</code>. However, for <B>TCP</B>
	 * transmitters, this may throw an <code>IOException</code> if the
	 * transmitter was already connected, therefore be sure to check
	 * <code>isConnected()</code> before.
	 * 
	 * @throws IOException
	 *             if a networking error occurs. Possible reasons: - the
	 *             underlying network channel had been closed by the server. -
	 *             the transport is TCP and the server is not available. - the
	 *             transport is TCP and an <code>OSCReceiver</code> sharing the
	 *             same socket was stopped before (unable to revive).
	 * 
	 * @see #isConnected()
	 */
	 ...
	 * 
	 * @return <code>true</code> if the transmitter is connected,
	...
	/**
	 * Sends an OSC packet (bundle or message) to the default network address,
	 * using the current codec. The default address is the one specified using
	 * the <code>setTarget</code> method. Therefore this will throw a
	...
	/**
	 * Sends an OSC packet (bundle or message) to the default network address,
	 * using a particular codec. The default address is the one specified using
	 * the <code>setTarget</code> method. Therefore this will throw a
	 * <code>NullPointerException</code> if no default address was specified.
	 * 
	 * @param c
