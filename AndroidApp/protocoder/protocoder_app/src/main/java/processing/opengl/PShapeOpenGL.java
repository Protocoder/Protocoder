...
public class PShapeOpenGL extends PShape {
	...
	protected PGL pgl;
	protected PGL.Context context; // The mainScriptContext that created this shape.
	...
	public PShapeOpenGL(PApplet parent, int family) {
		...
		// Initial values for fill, stroke and tint colors are also imported
		// from the renderer.
		// This is particular relevant for primitive shapes, since is not
		// possible to set
		// their color separately when creating them, and their input vertices
		// are actually
		// generated at rendering time, by which the color configuration of the
		// renderer might
		// have changed.
		fill = pg.fill;
	...
	public void end(int mode) {
		...
		// Input arrays are trimmed since they are expanded by doubling their
		// old size,
	...
	protected void updateStrokeWeight(float newWeight) {
		if (PGraphicsOpenGL.same(strokeWeight, newWeight))
			return;
	...
	protected void updateAmbientColor(int newAmbientColor) {
		if (ambientColor == newAmbientColor)
			return;
	...
	protected void updateEmissiveColor(int newEmissiveColor) {
		if (emissiveColor == newEmissiveColor)
			return;
	...
	protected void updateShininessFactor(float newShininess) {
		if (PGraphicsOpenGL.same(shininess, newShininess))
			return;
	...
	public void applyMatrix(float n00, float n01, float n02, float n10, float n11, float n12) {
		transform(MATRIX, n00, n01, n02, n10, n11, n12);
	}
	...
	public void applyMatrix(float n00, float n01, float n02, float n03, float n10, float n11, float n12, float n13,
		...
		transform(MATRIX, n00, n01, n02, n03, n10, n11, n12, n13, n20, n21, n22, n23, n30, n31, n32, n33);
	}
	...
	protected void calcTransform(int type, int dimensions, float... args) {
		...
		case MATRIX:
			if (dimensions == 3) {
				transform.set(args[0], args[1], args[2], args[3], args[4], args[5], args[6], args[7], args[8], args[9],
	...
	protected void bezierVertexImpl(float x2, float y2, float z2, float x3, float y3, float z3, float x4, float y4,
		...
		inGeo.addBezierVertex(x2, y2, z2, x3, y3, z3, x4, y4, z4, fill, stroke, bezierDetail, vertexCode(), kind);
	}
	...
	protected void quadraticVertexImpl(float cx, float cy, float cz, float x3, float y3, float z3) {
		inGeo.setMaterial(fillColor, strokeColor, strokeWeight, ambientColor, specularColor, emissiveColor, shininess);
		inGeo.setNormal(normalX, normalY, normalZ);
		inGeo.addQuadraticVertex(cx, cy, cz, x3, y3, z3, fill, stroke, bezierDetail, vertexCode(), kind);
	}
	...
	protected void curveVertexImpl(float x, float y, float z) {
		inGeo.setMaterial(fillColor, strokeColor, strokeWeight, ambientColor, specularColor, emissiveColor, shininess);
		inGeo.setNormal(normalX, normalY, normalZ);
		inGeo.addCurveVertex(x, y, z, fill, stroke, curveDetail, vertexCode(), kind);
	}
			...
			if (shapeEnded) {
				...
				if (family == GEOMETRY) {
					if (kind == POINTS) {
						...
						if (stroke)
							inGeo.addPolygonEdges(isClosed);
						tessellator.tessellatePolygon(isSolid, isClosed, normalMode == NORMAL_MODE_AUTO);
					}
				} else if (family == PRIMITIVE) {
					// The input geometry needs to be cleared because the
					// geometry
		...
		if (vertexCodeCount == 0) { // each point is a simple vertex
			...
			if (vertices[0].length == 2) { // tessellating a 2D path
				...
				for (int j = 0; j < vertexCodeCount; j++) {
					...
					case QUAD_BEZIER_VERTEX:
						inGeo.addQuadraticVertex(vertices[index + 0][X], vertices[index + 0][Y], 0,
								vertices[index + 1][X], vertices[index + 1][Y], 0, fill, stroke, bezierDetail, code);
						code = VERTEX;
						index += 2;
						break;
					...
					case BEZIER_VERTEX:
						inGeo.addBezierVertex(vertices[index + 0][X], vertices[index + 0][Y], 0,
								vertices[index + 1][X], vertices[index + 1][Y], 0, vertices[index + 2][X],
								vertices[index + 2][Y], 0, fill, stroke, bezierDetail, code);
						code = VERTEX;
						index += 3;
						break;
				...
				for (int j = 0; j < vertexCodeCount; j++) {
					...
					case QUAD_BEZIER_VERTEX:
						inGeo.addQuadraticVertex(vertices[index + 0][X], vertices[index + 0][Y],
								vertices[index + 0][Z], vertices[index + 1][X], vertices[index + 1][Y],
								vertices[index + 0][Z], fill, stroke, bezierDetail, code);
						code = VERTEX;
						index += 2;
						break;
					...
					case BEZIER_VERTEX:
						inGeo.addBezierVertex(vertices[index + 0][X], vertices[index + 0][Y], vertices[index + 0][Z],
								...
								vertices[index + 2][X], vertices[index + 2][Y], vertices[index + 2][Z], fill, stroke,
								bezierDetail, code);
						code = VERTEX;
						index += 3;
						break;
					...
					case CURVE_VERTEX:
						inGeo.addCurveVertex(vertices[index][X], vertices[index][Y], vertices[index][Z], fill, stroke,
								curveDetail, code);
						code = VERTEX;
						index++;
	...
	// updates the index cache for each child shape holding geometry (those
	// being
	// the leaf nodes in the hierarchy tree), and creates index caches for the
	// group shapes so that the draw() method can be called from any shape in
	// the
	...
	protected void updatePolyIndexCache() {
		...
		if (family == GROUP) {
			...
			// each subsequent level is determined by the higher-level group
			// shapes
			// The index pyramid is flattened into arrays in order to use simple
			// data structures, so each shape needs to store the positions in
			// the
			...
			for (int i = 0; i < childCount; i++) {
				...
				for (int n = first; n < first + count; n++) {
					if (gindex == -1) {
						...
						if (cache.vertexOffset[gindex] == cache.vertexOffset[n]) {
							// When the vertex offsets are the same, this means
							// that the
							// current index range in the group shape can be
							// extended to
							// include the index range in the current child
							// shape.
							// This is a result of how the indices are updated
							// for the
							// leaf shapes.
				...
				// Updating the first and last poly vertices for this group
				// shape.
			...
			// exceeded, therefore creating the start of a new index group in
			// the
			// root shape. When this happens, the indices in the child shape
			// need
			...
			for (int n = firstPolyIndexCache; n <= lastPolyIndexCache; n++) {
				...
				if (PGL.MAX_VERTEX_INDEX1 <= root.polyVertexRel + vcount || // Too
																			// many
																			// vertices
																			// already
																			// signal
																			...
																			// new
																			// cache...
						(is2D() && startStrokedTex(n))) { // ... or, in 2D, the
															// beginning of line
															// or points.
	...
	protected void updateLineIndexCache() {
		...
		if (family == GROUP) {
			...
			for (int i = 0; i < childCount; i++) {
				...
				// Updating the first and last line vertices for this group
				// shape.
	...
	protected void updatePointIndexCache() {
		...
		if (family == GROUP) {
			...
			for (int i = 0; i < childCount; i++) {
				...
				for (int n = first; n < first + count; n++) {
					if (gindex == -1) {
						...
						if (cache.vertexOffset[gindex] == cache.vertexOffset[n]) {
							...
							// current index range in the group shape can be
							// extended to
							// include either the index range in the current
							// child shape.
							// This is a result of how the indices are updated
							// for the
				...
				// Updating the first and last point vertices for this group
				// shape.
	...
	protected boolean contextIsOutdated() {
		...
		if (outdated) {
			...
			// automatically disposed when the old mainScriptContext was
			// destroyed.
			...
			// when the mainScriptContext changed. We only need to zero
	...
	protected void copyPolyVertices(int offset, int size) {
		pgl.bindBuffer(PGL.ARRAY_BUFFER, glPolyVertex);
		pgl.bufferSubData(PGL.ARRAY_BUFFER, 4 * offset * PGL.SIZEOF_FLOAT, 4 * size * PGL.SIZEOF_FLOAT,
	...
	protected void copyPolyColors(int offset, int size) {
		pgl.bindBuffer(PGL.ARRAY_BUFFER, glPolyColor);
		pgl.bufferSubData(PGL.ARRAY_BUFFER, offset * PGL.SIZEOF_INT, size * PGL.SIZEOF_INT,
	...
	protected void copyPolyNormals(int offset, int size) {
		pgl.bindBuffer(PGL.ARRAY_BUFFER, glPolyNormal);
		pgl.bufferSubData(PGL.ARRAY_BUFFER, 3 * offset * PGL.SIZEOF_FLOAT, 3 * size * PGL.SIZEOF_FLOAT,
	...
	protected void copyPolyTexcoords(int offset, int size) {
		pgl.bindBuffer(PGL.ARRAY_BUFFER, glPolyTexcoord);
		pgl.bufferSubData(PGL.ARRAY_BUFFER, 2 * offset * PGL.SIZEOF_FLOAT, 2 * size * PGL.SIZEOF_FLOAT,
	...
	protected void copyPolyShininess(int offset, int size) {
		pgl.bindBuffer(PGL.ARRAY_BUFFER, glPolyShininess);
		pgl.bufferSubData(PGL.ARRAY_BUFFER, offset * PGL.SIZEOF_FLOAT, size * PGL.SIZEOF_FLOAT,
	...
	protected boolean fragmentedGroup(PGraphics g) {
		return g.hintEnabled(ENABLE_ACCURATE_2D) || (textures != null && 1 < textures.size()) || strokedTexture;
	}
	...
	protected void render(PGraphicsOpenGL g, PImage texture) {
		if (root == null) {
			// Some error. Root should never be null. At least it should be
			// 'this'.
	...
	protected void renderPolys(PGraphicsOpenGL g, PImage textureImage) {
		...
		for (int n = firstPolyIndexCache; n <= lastPolyIndexCache; n++) {
			if (is3D()
					|| (tex != null && (firstLineIndexCache == -1 || n < firstLineIndexCache) && (firstPointIndexCache == -1 || n < firstPointIndexCache))) {
				...
				// Rendering line or point triangles, which are never lit nor
				// textured.
				if (!renderingStroke) {
					if (tex != null) {
						tex.unbind();
					...
					// If the renderer is 2D, then g.lights should always be
					// false,
			...
			shader.setVertexAttribute(root.glPolyVertex, 4, PGL.FLOAT, 0, 4 * voffset * PGL.SIZEOF_FLOAT);
			shader.setColorAttribute(root.glPolyColor, 4, PGL.UNSIGNED_BYTE, 0, 4 * voffset * PGL.SIZEOF_BYTE);

			if (g.lights) {
				shader.setNormalAttribute(root.glPolyNormal, 3, PGL.FLOAT, 0, 3 * voffset * PGL.SIZEOF_FLOAT);
				shader.setAmbientAttribute(root.glPolyAmbient, 4, PGL.UNSIGNED_BYTE, 0, 4 * voffset * PGL.SIZEOF_BYTE);
				shader.setSpecularAttribute(root.glPolySpecular, 4, PGL.UNSIGNED_BYTE, 0, 4 * voffset * PGL.SIZEOF_BYTE);
				shader.setEmissiveAttribute(root.glPolyEmissive, 4, PGL.UNSIGNED_BYTE, 0, 4 * voffset * PGL.SIZEOF_BYTE);
				shader.setShininessAttribute(root.glPolyShininess, 1, PGL.FLOAT, 0, voffset * PGL.SIZEOF_FLOAT);
			}

			if (tex != null) {
				shader.setTexcoordAttribute(root.glPolyTexcoord, 2, PGL.FLOAT, 0, 2 * voffset * PGL.SIZEOF_FLOAT);
			...
			for (int tr = ioffset / 3; tr < (ioffset + icount) / 3; tr++) {
				...
				if (textureImage != null) {
					...
					if (raw.is3D()) {
						...
						float sx0 = g.screenXImpl(pt0[0], pt0[1], pt0[2], pt0[3]), sy0 = g.screenYImpl(pt0[0], pt0[1],
								pt0[2], pt0[3]);
						float sx1 = g.screenXImpl(pt1[0], pt1[1], pt1[2], pt1[3]), sy1 = g.screenYImpl(pt1[0], pt1[1],
								pt1[2], pt1[3]);
						float sx2 = g.screenXImpl(pt2[0], pt2[1], pt2[2], pt2[3]), sy2 = g.screenYImpl(pt2[0], pt2[1],
								pt2[2], pt2[3]);
						raw.fill(argb0);
	...
	protected void renderLines(PGraphicsOpenGL g) {
		...
		for (int n = firstLineIndexCache; n <= lastLineIndexCache; n++) {
			...
			shader.setVertexAttribute(root.glLineVertex, 4, PGL.FLOAT, 0, 4 * voffset * PGL.SIZEOF_FLOAT);
			shader.setColorAttribute(root.glLineColor, 4, PGL.UNSIGNED_BYTE, 0, 4 * voffset * PGL.SIZEOF_BYTE);
			shader.setLineAttribute(root.glLineAttrib, 4, PGL.FLOAT, 0, 4 * voffset * PGL.SIZEOF_FLOAT);
	...
	protected void renderPoints(PGraphicsOpenGL g) {
		...
		for (int n = firstPointIndexCache; n <= lastPointIndexCache; n++) {
			...
			shader.setVertexAttribute(root.glPointVertex, 4, PGL.FLOAT, 0, 4 * voffset * PGL.SIZEOF_FLOAT);
			shader.setColorAttribute(root.glPointColor, 4, PGL.UNSIGNED_BYTE, 0, 4 * voffset * PGL.SIZEOF_BYTE);
			shader.setPointAttribute(root.glPointAttrib, 2, PGL.FLOAT, 0, 2 * voffset * PGL.SIZEOF_FLOAT);
			...
			while (pt < (ioffset + icount) / 3) {
				...
				if (0 < size) { // round point
					...
					perim = PApplet.max(PGraphicsOpenGL.MIN_POINT_ACCURACY,
							(int) (TWO_PI * weight / PGraphicsOpenGL.POINT_ACCURACY_FACTOR)) + 1;
				} else { // Square point
					weight = -size / 0.5f;
					perim = 5;
				}
