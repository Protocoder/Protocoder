...
public class PGL {
	...
	/**
	 * Count of tessellated fill, line or point vertices that will trigger a
	 * flush in the immediate mode. It doesn't necessarily be equal to
	...
	/**
	 * Minimum stroke weight needed to apply the full path stroking algorithm
	 * that properly generates caps and joing.
	 */
	...
	// Some EGL constants needed to initialize an GLES2 mainScriptContext.
	...
	protected String texVertShaderSource = "attribute vec2 inVertex;" + "attribute vec2 inTexcoord;"
			+ "varying vec2 vertTexcoord;" + "void main() {" + "  gl_Position = vec4(inVertex, 0, 1);"
			+ "  vertTexcoord = inTexcoord;" + "}";

	protected String texFragShaderSource = SHADER_PREPROCESSOR_DIRECTIVE + "uniform sampler2D textureSampler;"
			+ "varying vec2 vertTexcoord;" + "void main() {"
	...
	protected String rectVertShaderSource = "attribute vec2 inVertex;" + "void main() {"
			+ "  gl_Position = vec4(inVertex, 0, 1);" + "}";

	protected String rectFragShaderSource = SHADER_PREPROCESSOR_DIRECTIVE + "uniform vec4 rectColor;" + "void main() {"
	...
	protected void updatePrimary() {
		if (!initialized) {
			...
			// clearing
			// the screen in each frame. Because the way Android handles double
			// buffering
			...
			for (int i = 0; i < 2; i++) {
				...
				GLES20.glTexImage2D(GLES20.GL_TEXTURE_2D, 0, GLES20.GL_RGBA, fboWidth, fboHeight, 0, PGL.RGBA,
						PGL.UNSIGNED_BYTE, null);
			...
			if (packed) { // packed depth+stencil buffer
				...
				GLES20.glFramebufferRenderbuffer(GLES20.GL_FRAMEBUFFER, GLES20.GL_DEPTH_ATTACHMENT,
						GLES20.GL_RENDERBUFFER, depthStencil[0]);
	...
	protected void beginOnscreenDraw(boolean clear) {
		if (clear && !FORCE_SCREEN_FBO) {
			// Simplest scenario: clear mode means we clear both the color and
			// depth buffers.
			...
			// We need to save the color buffer after finishing with the
			// rendering of this frame,
			// to use is as the background for the next frame
			// ("incremental drawing").
			GLES20.glClearColor(0, 0, 0, 0);
			GLES20.glClear(GLES20.GL_DEPTH_BUFFER_BIT | GLES20.GL_STENCIL_BUFFER_BIT);
			if (firstOnscreenFrame) {
				// No need to draw back color buffer because we are in the first
				// frame.
				GLES20.glClear(GLES20.GL_COLOR_BUFFER_BIT);
			} else {
				// Render previous draw texture as background.
				drawTexture(GLES20.GL_TEXTURE_2D, glColorTex[backTex], fboWidth, fboHeight, 0, 0, pg.width, pg.height,
	...
	protected void endOnscreenDraw(boolean clear0) {
		if (!clear0 || FORCE_SCREEN_FBO) {
			// We are in the primary surface, and no clear mode, this means that
			// the current
			// contents of the front buffer needs to be used in the next frame
			// as the background.
			GLES20.glBindFramebuffer(GLES20.GL_FRAMEBUFFER, 0);
			...
			drawTexture(GLES20.GL_TEXTURE_2D, glColorTex[frontTex], fboWidth, fboHeight, 0, 0, pg.width, pg.height, 0,
	...
	public ByteBuffer mapBufferRange(int target, int offset, int length, int access) {
		// return gl2x.glMapBufferRange(GL.GL_ARRAY_BUFFER, offset, length,
		// GL2.GL_READ_WRITE);
		return null;
	}
	...
	public void blitFramebuffer(int srcX0, int srcY0, int srcX1, int srcY1, int dstX0, int dstY0, int dstX1, int dstY1,
			int mask, int filter) {
		// GLES20.glBlitFramebuffer(0, 0, srcW, srcH, 0, 0, destW, destH,
	...
	public void renderbufferStorageMultisample(int target, int samples, int format, int width, int height) {
		// GLES20.glRenderbufferStorageMultisample(GLES20.GL_RENDERBUFFER,
		// samples, format, w, h);
	}
	...
	public void drawTexture(int target, int id, int width, int height, int texX0, int texY0, int texX1, int texY1,
	...
	/**
	 * Converts input Java ARGB value to native OpenGL format (RGBA on big
	 * endian, BGRA on little endian).
	 */
	...
	/**
	 * Converts input Java ARGB value to native OpenGL format (RGBA on big
	 * endian, BGRA on little endian), setting alpha component to opaque (255).
	 */
	...
	public boolean validateFramebuffer() {
		...
		if (status == FRAMEBUFFER_COMPLETE) {
			...
			throw new RuntimeException("PFramebuffer: GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT ("
			...
			throw new RuntimeException("PFramebuffer: GL_FRAMEBUFFER_INCOMPLETE_FORMATS ("
					+ Integer.toHexString(status) + ")");
	...
	protected class AndroidConfigChooser implements EGLConfigChooser {
		...
		// The attributes we want in the frame buffer configuration for
		// Processing.
		...
		protected int[] configAttribsGL = { EGL10.EGL_RED_SIZE, 4, EGL10.EGL_GREEN_SIZE, 4, EGL10.EGL_BLUE_SIZE, 4,
		...
		public EGLConfig chooseConfig(EGL10 egl, EGLDisplay display) {
			...
			egl.eglChooseConfig(display, configAttribsGL, configs, numConfigs, num_config);

			if (PApplet.DEBUG) {
				for (EGLConfig config : configs) {
					String configStr = "P3D - selected EGL config : " + printConfig(egl, display, config);
		...
		public EGLConfig chooseBestConfig(EGL10 egl, EGLDisplay display, EGLConfig[] configs) {
			...
			for (EGLConfig config : configs) {
				...
				if (gl == EGL_OPENGL_ES2_BIT) {
					...
					float score = 0.20f * PApplet.abs(r - redTarget) + 0.20f * PApplet.abs(g - greenTarget) + 0.20f
							* PApplet.abs(b - blueTarget) + 0.15f * PApplet.abs(a - blueTarget) + 0.15f
							* PApplet.abs(d - depthTarget) + 0.10f * PApplet.abs(s - stencilTarget);

					if (score < bestScore) {
						// We look for the config closest to the target config.
						// Closeness is measured by the score function defined
						// above:
						// we give more weight to the RGB components, followed
						// by the
