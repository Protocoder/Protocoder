...
public class Texture implements PConstants {
	...
	protected PGL.Context context; // The mainScriptContext that created this texture.
	...
	public void set(int texTarget, int texName, int texWidth, int texHeight, int target, int tex, int x, int y, int w,
			int h) {
	...
	public void set(int[] pixels, int x, int y, int w, int h, int format) {
		...
		if (pixels.length == 0) {
			// Nothing to do (means that w == h == 0) but not an erroneous
			// situation
			return;
		}
		...
		if (usingMipmaps) {
			if (PGraphicsOpenGL.autoMipmapGenSupported) {
				...
				// TODO: finish manual mipmap generation, replacing Bitmap with
				// AWT's BufferedImage,
				// making it work in npot textures (embed npot tex into larger
				// pot tex?), subregions,
				// and moving GLUtils.texImage2D (originally from Android SDK)
				// into PGL.
				// Actually, this whole code should go into PGL, so the Android
				// implementation can
				// use Bitmap, and desktop use BufferedImage.

				/*
				 * if (w != width || h != height) { System.err.println(
				 * "Sorry but I don't know how to generate mipmaps for a subregion."
				 * ); return; }
				 * 
				 * // Code by Mike Miller obtained from here: //
				 * http://insanitydesign
				 * .com/wp/2009/08/01/android-opengl-es-mipmaps/ int w0 =
				 * glWidth; int h0 = glHeight; int[] argbPixels = new int[w0 *
				 * h0]; convertToARGB(pixels, argbPixels, format); int level =
				 * 0; int denom = 1;
				 * 
				 * // We create a Bitmap because then we use its built-in
				 * filtered downsampling // functionality. Bitmap bitmap =
				 * Bitmap.createBitmap(w0, h0, Config.ARGB_8888);
				 * bitmap.setPixels(argbPixels, 0, w0, 0, 0, w0, h0);
				 * 
				 * while (w0 >= 1 || h0 >= 1) { //First of all, generate the
				 * texture from our bitmap and set it to the according level
				 * GLUtils.texImage2D(glTarget, level, bitmap, 0);
				 * 
				 * // We are done. if (w0 == 1 && h0 == 1) { break; }
				 * 
				 * // Increase the mipmap level level++; denom *= 2;
				 * 
				 * // Downsampling bitmap. We must eventually arrive to the 1x1
				 * level, // and if the width and height are different, there
				 * will be a few 1D // texture levels just before. // This
				 * update formula also allows for NPOT resolutions. w0 =
				 * PApplet.max(1, PApplet.floor((float)glWidth / denom)); h0 =
				 * PApplet.max(1, PApplet.floor((float)glHeight / denom)); //
				 * (see getScaledInstance in AWT Image) Bitmap bitmap2 =
				 * Bitmap.createScaledBitmap(bitmap, w0, h0, true);
				 * 
				 * // Clean up bitmap.recycle(); bitmap = bitmap2; }
				 */
	...
	public void setNative(IntBuffer pixels, int x, int y, int w, int h) {
		...
		if (pixels.capacity() != w * h) {
			PGraphics.showWarning("The pixels array has a length of " + pixels.capacity() + ", but it should be " + w
					* h);
			return;
		}
	...
	public void get(int[] pixels) {
		...
		// Attaching the texture to the color buffer of a FBO, binding the FBO
		// and reading the pixels
	...
	public void loadPixels(int[] pixels) {
		if (hasBuffers()) {
			// Updates the texture AND the pixels array of the image at the same
			// time,
			// getting the pixels directly from the buffer data (and thus
			// avoiding expensive
	...
	public void put(int texTarget, int texName, int texWidth, int texHeight, int target, int tex, int x, int y, int w,
			int h) {
	...
	public void copyBufferFromSource(Object natRef, ByteBuffer byteBuf, int w, int h) {
		...
		if (bufferCache.size() + 1 <= MAX_BUFFER_CACHE_SIZE) {
			...
			// The buffer cache reached the maximum size, so we just dispose the
			// new buffer.
			try {
	 ...
	 * OpenGL (RGBA). Both arrays are assumed to be of the same length. The
	 * width and height parameters are used in the YUV420 to RBGBA conversion.
	 * 
	 * @param intArray
	 *            int[]
	 * @param tIntArray
	...
	protected void convertToRGBA(int[] intArray, int[] tIntArray, int arrayFormat, int w, int h) {
		if (PGL.BIG_ENDIAN) {
			...
			case RGB:
				...
				// Converting xRGB into RGBA. A is set to 0xFF (255, full
				// opacity).
				for (int i = 0; i < intArray.length; i++) {
			...
			case ARGB:
				...
				// Converting ARGB into RGBA. Shifting RGB to 8 bits to the
				// left,
			...
			case RGB:
				...
				for (int i = 0; i < intArray.length; i++) {
					int pixel = intArray[i];
					tIntArray[i] = 0xFF000000 | ((pixel & 0xFF) << 16) | ((pixel & 0xFF0000) >> 16)
							| (pixel & 0x0000FF00);
				}
				break;
	...
	/**
	 * Reorders a pixel array in a given format into ARGB. The input array must
	 * be of size width * height, while the output array must be of glWidth *
	 * glHeight.
	 * 
	 * @param intArray
	 *            int[]
	 ...
	 */
	/*
	 * protected void convertToARGB(int[] intArray, int[] tIntArray, int
	 * arrayFormat) { int t = 0; int p = 0; int pixel;
	 * 
	 * switch (arrayFormat) { case ALPHA:
	 * 
	 * // xxxA to ARGB, setting RGB to black. for (int y = 0; y < height; y++) {
	 * for (int x = 0; x < width; x++) { pixel = intArray[p++]; tIntArray[t++] =
	 * (pixel << 24) & 0xFF000000; } t += glWidth - width; }
	 * 
	 * break;
	 * 
	 * case RGB:
	 * 
	 * // xRGB to ARGB, setting A to be 0xFF. for (int y = 0; y < height; y++) {
	 * for (int x = 0; x < width; x++) { pixel = intArray[p++]; tIntArray[t++] =
	 * pixel | 0xFF000000; } t += glWidth - width; }
	 * 
	 * break;
	 * 
	 * case ARGB:
	 * 
	 * // ARGB to ARGB, where the source is smaller than the destination. for
	 * (int y = 0; y < height; y++) { PApplet.arrayCopy(intArray, width * y,
	 * tIntArray, glWidth * y, width); }
	 * 
	 * break;
	 * 
	 * }
	...
	protected void setSize(int w, int h) {
		...
		if ((glWidth > PGraphicsOpenGL.maxTextureSize) || (glHeight > PGraphicsOpenGL.maxTextureSize)) {
			...
			throw new RuntimeException("Image width and height cannot be" + " larger than "
					+ PGraphicsOpenGL.maxTextureSize + " with this graphics card.");
		}

		// If non-power-of-two textures are not supported, and the specified
		// width or height
		// is non-power-of-two, then glWidth (glHeight) will be greater than w
		// (h) because it
		// is chosen to be the next power of two, and this quotient will give
		// the appropriate
	...
	protected void allocate() {
		...
		// First, we use glTexImage2D to set the full size of the texture
		// (glW/glH might be diff
	...
	protected boolean contextIsOutdated() {
		...
		if (outdated) {
			...
			// automatically disposed when the old mainScriptContext was destroyed.
	...
	protected void copyTexture(Texture tex, int x, int y, int w, int h, boolean scale) {
		...
		if (scale) {
			...
			pgl.drawTexture(tex.glTarget, tex.glName, tex.glWidth, tex.glHeight, x, y, w, h, 0, 0, width, height);

		} else {
			// Rendering tex into "this" but without scaling so the contents
			// of the source texture fall in the corresponding texels of the
			// destination.
			pgl.drawTexture(tex.glTarget, tex.glName, tex.glWidth, tex.glHeight, x, y, w, h, x, y, w, h);
		}
		pg.popFramebuffer();
		updateTexels(x, y, w, h);
	}
	...
	// Copies source texture tex into this.
	protected void copyTexture(int texTarget, int texName, int texWidth, int texHeight, int x, int y, int w, int h,
			boolean scale) {
		if (tempFbo == null) {
			tempFbo = new FrameBuffer(parent, glWidth, glHeight);
		}
