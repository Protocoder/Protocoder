 ...
 * fragment shader. Originally based in the code by JohnG
...
public class PShader {
	...
	protected PGL pgl;
	protected PGL.Context context; // The mainScriptContext that created this shader.
	...
	public void set(String name, PMatrix2D mat) {
		float[] matv = { mat.m00, mat.m01, mat.m10, mat.m11 };
	...
	public void set(String name, PMatrix3D mat, boolean use3x3) {
		if (use3x3) {
			float[] matv = { mat.m00, mat.m01, mat.m02, mat.m10, mat.m11, mat.m12, mat.m20, mat.m21, mat.m22 };
			setUniformImpl(name, UniformValue.MAT3, matv);
		} else {
			float[] matv = { mat.m00, mat.m01, mat.m02, mat.m03, mat.m10, mat.m11, mat.m12, mat.m13, mat.m20, mat.m21,
					mat.m22, mat.m23, mat.m30, mat.m31, mat.m32, mat.m33 };
	 ...
	 * http://stackoverflow.com/questions/7718976/what-is-glvertexattrib-versus-
	 * glvertexattribpointer-used-for // except for setting a constant vertex
	 * attribute value. public void set1FloatAttribute(int loc, float x) { if
	 * (-1 < loc) { pgl.glVertexAttrib1f(loc, x); } }
	 * 
	 * 
	 * public void set2FloatAttribute(int loc, float x, float y) { if (-1 < loc)
	 * { pgl.glVertexAttrib2f(loc, x, y); } }
	 * 
	 * 
	 * public void set3FloatAttribute(int loc, float x, float y, float z) { if
	 * (-1 < loc) { pgl.glVertexAttrib3f(loc, x, y, z); } }
	 * 
	 * 
	 * public void set4FloatAttribute(int loc, float x, float y, float z, float
	 * w) { if (-1 < loc) { pgl.glVertexAttrib4f(loc, x, y, z, w); } }
	 */
	...
	protected void init() {
		if (glProgram == 0 || contextIsOutdated()) {
			...
			boolean vertRes = false;
			if (hasVert) {
				vertRes = compileVertexShader();
			}

			boolean fragRes = false;
			if (hasFrag) {
				fragRes = compileFragmentShader();
			}

			Log.d("qq", "compiled " + vertRes + " " + fragRes);
	...
	protected boolean loadFragmentShader(String filename) {
		...
		String[] fileShader = parent.loadStrings(filename);
		if (fileShader != null) {
			fragmentShaderSource = PApplet.join(fileShader, "\n");
			Log.d("qq: ", "fragmentShaderSource " + fragmentShaderSource);
