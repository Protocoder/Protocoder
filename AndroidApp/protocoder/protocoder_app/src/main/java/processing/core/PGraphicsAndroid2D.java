 ...
 * graphics model. Similar tradeoffs to JAVA2D mode with the original (desktop)
...
public class PGraphicsAndroid2D extends PGraphics {
	...
	/*
	 * public void requestDraw() { parent.surfaceView.requestRender(); }
	 */
	...
	public void vertex(float x, float y) {
		...
		if (shape == POLYGON) {
			...
			// this is way too slow, otherwise what's the point of using
			// beginShape()
			...
			case TRIANGLE_STRIP:
				if (vertexCount >= 3) {
					triangle(vertices[vertexCount - 2][X], vertices[vertexCount - 2][Y], x, // vertices[vertexCount
																							// -
																							// 1][X],
							y, // vertices[vertexCount - 1][Y],
							vertices[vertexCount - 3][X], vertices[vertexCount - 3][Y]);
				}
				break;
			...
			case TRIANGLE_FAN:
				if (vertexCount == 3) {
					...
					path.lineTo(vertices[vertexCount - 2][X], vertices[vertexCount - 2][Y]);
					path.lineTo(x, y);
					drawPath();
				}
				break;
			...
			case QUADS:
				if ((vertexCount % 4) == 0) {
					quad(vertices[vertexCount - 4][X], vertices[vertexCount - 4][Y], vertices[vertexCount - 3][X],
							vertices[vertexCount - 3][Y], vertices[vertexCount - 2][X], vertices[vertexCount - 2][Y],
							x, y);
					vertexCount = 0;
				}
				break;
	...
	public void point(float x, float y) {
		...
		// set(PApplet.round(screenX(x, y)), PApplet.round(screenY(x, y)),
		// strokeColor);
		// }
	}
	...
	protected void arcImpl(float x, float y, float w, float h, float start, float stop) {
		...
		if (stop - start >= TWO_PI) {
			...
			// Android agrees with us, so don't set start/stop negative like
			// Java 2D
	...
	protected void imageImpl(PImage src, float x1, float y1, float x2, float y2, int u1, int v1, int u2, int v2) {
		...
		// this version's not usable because it doesn't allow you to set output
		// w/h
		...
		if (src.modified) {
			// System.out.println("mutable, recycled = " +
			// who.bitmap.isMutable() + ", " + who.bitmap.isRecycled());
		...
		// canvas.drawBitmap(who.bitmap, imageImplSrcRect, imageImplDstRect,
		// tint ? tintPaint : null);
		// System.out.println(PApplet.hex(fillPaint.getColor()));
		// canvas.drawBitmap(who.bitmap, imageImplSrcRect, imageImplDstRect,
		// fillPaint);
		// System.out.println("drawing lower, tint = " + tint + " " +
		// PApplet.hex(tintPaint.getColor()));
	...
	protected void textLineImpl(char buffer[], int start, int stop, float x, float y) {
		 ...
		 * fixed a bug in 0116 that handled this incorrectly. Object
		 * textAntialias =
		...
		// canvas.setColor(fillColorObject);
		int length = stop - start;
		// canvas.drawChars(buffer, start, length, (int) (x + 0.5f), (int) (y +
		// 0.5f));
	...
	public PMatrix2D getMatrix(PMatrix2D target) {
		...
		// target.set((float) transform[0], (float) transform[2], (float)
		// transform[4],
		// (float) transform[1], (float) transform[3], (float) transform[5]);
		target.set((float) transform[0], (float) transform[1], (float) transform[2], (float) transform[3],
