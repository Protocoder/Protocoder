...
public class PShape implements PConstants {
	 ...
	 * if not. This is modified with the <b>setVisible()</b> parameter. <br>
	 * <br>
	 ...
	 * value of the <b>visible</b> parameter. <br>
	 * <br>
	 * The visibility of a shape is usually controlled by whatever program
	 * created the SVG file. For instance, this parameter is controlled by
	 * showing or hiding the shape in the layers palette in Adobe Illustrator.
	 ...
	 * joints. =advanced Overrides this shape's style information and uses
	 * PGraphics styles and colors. Identical to ignoreStyles(true). Also
	 ...
	 * Only makes sense for 3D PShape subclasses, such as PShape3D.
	 */
	...
	/*
	 * boolean strokeSaved; int strokeColorSaved; float strokeWeightSaved; int
	 * strokeCapSaved; int strokeJoinSaved;
	 * 
	...
	protected void pre(PGraphics g) {
		...
		/*
		 * strokeSaved = g.stroke; strokeColorSaved = g.strokeColor;
		 * strokeWeightSaved = g.strokeWeight; strokeCapSaved = g.strokeCap;
		 * strokeJoinSaved = g.strokeJoin;
		 * 
		 * fillSaved = g.fill; fillColorSaved = g.fillColor;
		 * 
		 * rectModeSaved = g.rectMode; ellipseModeSaved = g.ellipseMode;
	...
	public void post(PGraphics g) {
		 ...
		 * g.stroke = strokeSaved; g.strokeColor = strokeColorSaved;
		 * g.strokeWeight = strokeWeightSaved; g.strokeCap = strokeCapSaved;
		 * g.strokeJoin = strokeJoinSaved;
		 * 
		 * g.fill = fillSaved; g.fillColor = fillColorSaved;
		 * 
	...
	static public void copyGeometry(PShape src, PShape dest) {
		...
		if (src.style) {
			for (int i = 0; i < src.vertexCount; i++) {
				...
				// s.specular(vert[SPR] * 255, vert[SPG] * 255, vert[SPB] *
				// 255);
	...
	/*
	 * protected void drawPath(PGraphics g) { g.beginShape(); for (int j = 0; j
	 * < childCount; j++) { if (j > 0) g.breakShape(); int count =
	 * children[j].vertexCount; float[][] vert = children[j].vertices; int[]
	 * code = children[j].vertexCodes;
	 * 
	 * for (int i = 0; i < count; i++) { if (style) { if (children[j].fill) {
	 * g.fill(vert[i][R], vert[i][G], vert[i][B]); } else { g.noFill(); } if
	 * (children[j].stroke) { g.stroke(vert[i][R], vert[i][G], vert[i][B]); }
	 * else { g.noStroke(); } } g.edge(vert[i][EDGE] == 1);
	 * 
	 * if (code[i] == VERTEX) { g.vertex(vert[i]);
	 * 
	 * } else if (code[i] == BEZIER_VERTEX) { float z0 = vert[i+0][Z]; float z1
	 * = vert[i+1][Z]; float z2 = vert[i+2][Z]; if (z0 == 0 && z1 == 0 && z2 ==
	 * 0) { g.bezierVertex(vert[i+0][X], vert[i+0][Y], z0, vert[i+1][X],
	 * vert[i+1][Y], z1, vert[i+2][X], vert[i+2][Y], z2); } else {
	 * g.bezierVertex(vert[i+0][X], vert[i+0][Y], vert[i+1][X], vert[i+1][Y],
	 * vert[i+2][X], vert[i+2][Y]); } } else if (code[i] == CURVE_VERTEX) {
	 * float z = vert[i][Z]; if (z == 0) { g.curveVertex(vert[i][X],
	 * vert[i][Y]); } else { g.curveVertex(vert[i][X], vert[i][Y], z); } } } }
	 * g.endShape(); }
	 */
			...
			if (vertices[0].length == 2) { // drawing a 2D path
				for (int j = 0; j < vertexCodeCount; j++) {
					...
					case QUAD_BEZIER_VERTEX:
						g.quadraticVertex(vertices[index + 0][X], vertices[index + 0][Y], vertices[index + 1][X],
								vertices[index + 1][Y]);
					...
					case BEZIER_VERTEX:
						g.bezierVertex(vertices[index + 0][X], vertices[index + 0][Y], vertices[index + 1][X],
								vertices[index + 1][Y], vertices[index + 2][X], vertices[index + 2][Y]);
			...
			for (int i = 0, j = vertexCount - 1; i < vertexCount; j = i++) {
				if (((vertices[i][Y] > y) != (vertices[j][Y] > y))
						&& (x < (vertices[j][X] - vertices[i][X]) * (y - vertices[i][Y])
								/ (vertices[j][1] - vertices[i][Y]) + vertices[i][X])) {
					c = !c;
				}
			}
			return c;
	 ...
	 * it's not refreshed each time <b>draw()</b> is run. <br>
	 * <br>
	 * This method requires a 3D renderer. You need to pass P3D or OPENGL as a
	 * third parameter into the <b>size()</b> method as shown in the example
	 * above.
	 * 
	 * @param angle
	 ...
	 * method call <b>scale(2.0)</b> increases the dimension of a shape by 200%.
	...
	public void applyMatrix(PMatrix2D source) {
		applyMatrix(source.m00, source.m01, 0, source.m02, source.m10, source.m11, 0, source.m12, 0, 0, 1, 0, 0, 0, 0,
				1);
	}

	public void applyMatrix(float n00, float n01, float n02, float n10, float n11, float n12) {
		checkMatrix(2);
		matrix.apply(n00, n01, n02, 0, n10, n11, n12, 0, 0, 0, 1, 0, 0, 0, 0, 1);
	}
	...
	/**
	 * Center the shape based on its bounding box. Can't assume that the
	 * bounding box is 0, 0, width, height. Common case will be opening a letter
	 * size document in Illustrator, and drawing something in the middle, then
	 * reading it in as an svg file. This will also need to flip the y axis
	 * (scale(1, -1)) in cases like Adobe Illustrator where the coordinates
	 * start at the bottom.
	 */
	// public void center() {
	// }
	 ...
	 * @param maxY
	 *            range for the green or saturation depending on the current
	 *            color mode
	 * @param maxZ
	 *            range for the blue or brightness depending on the current
	 *            color mode
	 */
