...
/**
 * Main graphics and rendering mainScriptContext, as well as the base API implementation.
 * 
...
public class PGraphics extends PImage implements PConstants {
	...
	/**
	 * Last value set by strokeWeight() (read-only). This has a default setting,
	 * rather than fighting with renderers about whether that renderer supports
	 * thick lines.
	 */
	public float strokeWeight = DEFAULT_STROKE_WEIGHT;

	/**
	 * Set by strokeJoin() (read-only). This has a default setting so that
	 * strokeJoin() need not be called by defaults, because subclasses may not
	 * implement it (i.e. PGraphicsGL)
	 */
	public int strokeJoin = DEFAULT_STROKE_JOIN;

	/**
	 * Set by strokeCap() (read-only). This has a default setting so that
	 * strokeCap() need not be called by defaults, because subclasses may not
	 * implement it (i.e. PGraphicsGL)
	 */
	 ...
	 * this will be associated with their MemoryImageSource. For
	 * PGraphicsJava2D, it will be the offscreen drawing buffer.
	 */
	// public Image image;
	...
	/**
	 * Internal buffer used by the text() functions because the String object is
	 * slow
	 */
	...
	/**
	 * Handle any takedown for this graphics mainScriptContext.
	 * <p>
	...
	/**
	 * Set engine's default values. This has to be called by PApplet, somewhere
	 * inside setup() or draw() because it talks to the graphics buffer, meaning
	 * that for subclasses like OpenGL, there needs to be a valid graphics
	 * mainScriptContext to mess with otherwise you'll get some good crashing action.
	 * 
	 ...
	 * variable) because they affect the graphics mainScriptContext, or they require
	 * parameters from the mainScriptContext (e.g. getting native fonts for text).
	 * 
	...
	public void vertex(float x, float y, float z) {
		...
		if (fill || textured) {
			...
			/*
			 * vertex[AR] = ambientR; vertex[AG] = ambientG; vertex[AB] =
			 * ambientB;
			 * 
			 * vertex[SPR] = specularR; vertex[SPG] = specularG; vertex[SPB] =
			 * specularB; //vertex[SPA] = specularA;
			 * 
			 * vertex[SHINE] = shininess;
			 * 
			 * vertex[ER] = emissiveR; vertex[EG] = emissiveG; vertex[EB] =
			 * emissiveB;
			 */
		}
	...
	/**
	 * Internal method to copy all style information for the given vertex. Can
	 * be overridden by subclasses to handle only properties pertinent to that
	 * renderer. (e.g. no need to copy the emissive color in P2D)
	 */
	 ...
	 * overridden by subclasses that need more efficient rendering options.
	 */
	protected void curveVertexSegment(float x1, float y1, float z1, float x2, float y2, float z2, float x3, float y3,
	...
	public float curvePoint(float a, float b, float c, float d, float t) {
		...
		return (a * (ttt * cb.m00 + tt * cb.m10 + t * cb.m20 + cb.m30) + b
				* (ttt * cb.m01 + tt * cb.m11 + t * cb.m21 + cb.m31) + c
				* (ttt * cb.m02 + tt * cb.m12 + t * cb.m22 + cb.m32) + d
	...
	public float curveTangent(float a, float b, float c, float d, float t) {
		...
		// not optimized (and probably need not be)
		return (a * (tt3 * cb.m00 + t2 * cb.m10 + cb.m20) + b * (tt3 * cb.m01 + t2 * cb.m11 + cb.m21) + c
				* (tt3 * cb.m02 + t2 * cb.m12 + cb.m22) + d * (tt3 * cb.m03 + t2 * cb.m13 + cb.m23));
	}
	...
	public void textFont(PFont which) {
		if (which != null) {
			 ...
			 * 
			 * //textFontNativeMetrics = null; // changed for rev 0104 for
			 * textMode(SHAPE) in opengl if (textFontNative != null) { // TODO
			 * need a better way to handle this. could use reflection to get //
			 * rid of the warning, but that'd be a little silly. supporting this
			 * is // an artifact of supporting java 1.1, otherwise we'd use
			 * getLineMetrics, // as recommended by the @deprecated flag.
			 * textFontNativeMetrics =
			 * Toolkit.getDefaultToolkit().getFontMetrics(textFontNative); //
			 * The following is what needs to be done, however we need to be
			 * able // to get the actual graphics mainScriptContext where the drawing is
			 * happening. // For instance, parent.getGraphics() doesn't work for
			 * OpenGL since // an OpenGL drawing surface is an embedded
			 * component. // if (parent != null) { // textFontNativeMetrics =
	...
	/**
	 * Implementation of returning the text width of the chars [start, stop) in
	 * the buffer. Unlike the previous version that was inside PFont, this will
	 ...
	 */
	private boolean textSentence(char[] buffer, int start, int stop, float boxWidth, float spaceWidth) {
		float runningX = 0;
	...
	// public void text(String s, float x1, float y1, float x2, float y2, float
	// z) {
	...
	/**
	 * Handles placement of a text line, then calls textLineImpl to actually
	 * render at the specific point.
	 */
	...
	protected void textCharModelImpl(PImage glyph, float x1, float y1, // float
																		// z1,
	...
	public void style(PStyle s) {
		...
		/*
		 * s.ambientR = ambientR; s.ambientG = ambientG; s.ambientB = ambientB;
		 * s.specularR = specularR; s.specularG = specularG; s.specularB =
		 * specularB; s.emissiveR = emissiveR; s.emissiveG = emissiveG;
		 * s.emissiveB = emissiveB; s.shininess = shininess;
		 */
	...
	static public int lerpColor(int c1, int c2, float amt, int mode) {
		if (mode == RGB) {
			...
			/*
			 * If mode is HSB, this will take the shortest path around the color
			 * wheel to find the new color. For instance, red to blue will go
			 * red violet blue (backwards in hue space) rather than cycling
			 * through ROYGBIV.
			 */
			...
			// direction
			/*
			 * float h1 = lerpColorHSB1[0]; float h2 = lerpColorHSB2[0]; if
			 * (Math.abs(h1 - h2) > 0.5f) { if (h1 > h2) { // i.e. h1 is 0.7, h2
			 * is 0.1 h2 += 1; } else { // i.e. h1 is 0.1, h2 is 0.7 h1 += 1; }
			 * } float ho = (PApplet.lerp(lerpColorHSB1[0], lerpColorHSB2[0],
			 * amt)) % 1.0f;
			 */

			// float ho = PActivity.lerp(lerpColorHSB1[0], lerpColorHSB2[0],
			// amt);
			// float so = PActivity.lerp(lerpColorHSB1[1], lerpColorHSB2[1],
			// amt);
			// float bo = PActivity.lerp(lerpColorHSB1[2], lerpColorHSB2[2],
			// amt);
	...
	/**
	 * Same as below, but defaults to a 12 point font, just as MacWrite
	 * intended.
	 */
	...
	/**
	 * First try to create a default font, but if that's not possible, throw an
	 * exception that halts the program because textFont() has not been used
