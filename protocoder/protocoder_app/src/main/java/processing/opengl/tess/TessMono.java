...
class TessMono {
	/*
	 * __gl_meshTessellateMonoRegion( face ) tessellates a monotone region (what
	 * else would it do??) The region must consist of a single loop of
	 * half-edges (see mesh.h) oriented CCW. "Monotone" in this case means that
	 * any vertical line intersects the interior of the region in a single
	 * interval.
	 * 
	 * Tessellation consists of adding interior edges (actually pairs of
	 * half-edges), to split the region into non-overlapping triangles.
	 * 
	 * The basic idea is explained in Preparata and Shamos (which I don''t have
	 * handy right now), although their implementation is more complicated than
	 * this one. The are two edge chains, an upper chain and a lower chain. We
	 * process all vertices from both chains in order, from right to left.
	 * 
	...
	static boolean __gl_meshTessellateMonoRegion(GLUface face, boolean avoidDegenerateTris) {
		...
		/*
		 * All edges are oriented CCW around the boundary of the region. First,
		 * find the half-edge whose origin vertex is rightmost. Since the sweep
		 * goes from left to right, face->anEdge should be close to the edge we
		 * want.
		 */
		up = face.anEdge;
		...
		while (up.Lnext != lo) {
			...
			if (Geom.VertLeq(up.Sym.Org, lo.Org)) {
				/*
				 * up.Sym.Org is on the left. It is safe to form triangles from
				 * lo.Org. The EdgeGoesLeft test guarantees progress even when
				 * some triangles are CW, given that the upper and lower chains
				 * are truly monotone.
				 */
				while (lo.Lnext != up
				...
				/*
				 * lo.Org is on the left. We can make CCW triangles from
				 * up.Sym.Org.
				 */
				while (lo.Lnext != up
	...
	/*
	 * __gl_meshTessellateInterior( mesh ) tessellates each region of the mesh
	 * which is marked "inside" the polygon. Each such region must be monotone.
	 */
	public static boolean __gl_meshTessellateInterior(GLUmesh mesh, boolean avoidDegenerateTris) {
		...
		for (f = mesh.fHead.next; f != mesh.fHead; f = next) {
			...
			next = f.next;
			if (f.inside) {
				if (!__gl_meshTessellateMonoRegion(f, avoidDegenerateTris))
					return false;
			}
		}
	...
	/*
	 * __gl_meshDiscardExterior( mesh ) zaps (ie. sets to NULL) all faces which
	 * are not marked "inside" the polygon. Since further mesh operations on
	 * NULL faces are not allowed, the main purpose is to clean up the mesh so
	 * that exterior loops are not represented in the data structure.
	 */
	 ...
	 * winding numbers on all edges so that regions marked "inside" the polygon
	 * have a winding number of "value", and regions outside have a winding
	 * number of 0.
	 * 
	...
	public static boolean __gl_meshSetWindingNumber(GLUmesh mesh, int value, boolean keepOnlyBoundary) {
		...
		for (e = mesh.eHead.next; e != mesh.eHead; e = eNext) {
			...
			if (e.Sym.Lface.inside != e.Lface.inside) {
				...
				/*
				 * This is a boundary edge (one side is interior, one is
				 * exterior).
				 */
