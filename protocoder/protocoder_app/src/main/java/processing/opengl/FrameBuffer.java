...
public class FrameBuffer implements PConstants {
	...
	protected PGL pgl;
	protected PGL.Context context; // The mainScriptContext that created this framebuffer.
	...
	FrameBuffer(PApplet parent, int w, int h, int samples, int colorBuffers, int depthBits, int stencilBits,
			boolean packedDepthStencil, boolean screen) {
		this.parent = parent;
		...
		if (depthBits < 1 && stencilBits < 1) {
			...
			if (packedDepthStencil) {
				// When combined depth/stencil format is required, the depth and
				// stencil bits
				// are overriden and the 24/8 combination for a 32 bits surface
				// is used.
				this.depthBits = 24;
				this.stencilBits = 8;
	...
	public void copy(FrameBuffer dest) {
		...
		pgl.blitFramebuffer(0, 0, this.width, this.height, 0, 0, dest.width, dest.height, PGL.COLOR_BUFFER_BIT,
				PGL.NEAREST);
	}
	...
	public void setColorBuffers(Texture[] textures, int n) {
		...
		for (int i = 0; i < numColorBuffers; i++) {
			pgl.framebufferTexture2D(PGL.FRAMEBUFFER, PGL.COLOR_ATTACHMENT0 + i, colorBufferTex[i].glTarget,
